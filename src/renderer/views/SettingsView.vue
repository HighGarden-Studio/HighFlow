...
// Local state
const activeTab = ref<'general' | 'ai' | 'mcp' | 'agents' | 'integrations' | 'shortcuts' | 'profile' | 'data'>('general');
const appInfo = ref<{ name: string; version: string; platform: string; isDev: boolean } | null>(null);
...
function getTagColor(tag: AIProviderTag): string {
  const colors: Record<string, string> = {
    ...
    'image-analysis': 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300',
...
// Tab definitions
type TabId = 'general' | 'ai' | 'mcp' | 'agents' | 'integrations' | 'shortcuts' | 'profile' | 'data';
const tabs: { id: TabId; label: string; icon: string }[] = [
  ...
  { id: 'mcp', label: 'MCP Servers', icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z' },
  { id: 'agents', label: 'Local Agents', icon: 'M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
  ...
  { id: 'shortcuts', label: 'Shortcuts', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
...
<template>
  <div class="flex-1 flex flex-col h-full bg-gray-50 dark:bg-gray-900">
    ...
    <div class="flex-1 flex overflow-hidden">
      <!-- Sidebar Navigation -->
      <nav class="w-56 flex-shrink-0 border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
        <ul class="space-y-1">
      ...
      <main class="flex-1 overflow-y-auto p-6">
        <div class="max-w-3xl">
          ...
          <div v-if="activeTab === 'general'" class="space-y-6">
            ...
            <section>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Behavior</h2>
              <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm divide-y divide-gray-200 dark:divide-gray-700">
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Auto-save</h3>
                ...
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Show Task IDs</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Display task IDs in the interface</p>
                  </div>
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Enable Animations</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Use animations and transitions</p>
                  </div>
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Compact Mode</h3>
                ...
                <div class="grid grid-cols-4 gap-2">
                  <button
                    ...
                    :class="[
                      ...
                      settingsStore.generalSettings.defaultProjectView === view
                        ? 'bg-blue-600 text-white'
                        : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
                    ]"
                    @click="settingsStore.updateGeneralSettings({ defaultProjectView: view as any })"
                  >
                    {{ view }}
                  </button>
                </div>
          ...
          <div v-if="activeTab === 'ai'" class="space-y-6">
            <section>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">AI Providers</h2>
              <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                Configure your AI provider credentials to enable AI-powered features.
                Multiple providers can be enabled - the system will fallback to the next available provider if one fails.
              </p>

              <!-- Enabled Providers Summary -->
              <div v-if="settingsStore.enabledProviders.length > 0" class="mb-4 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <p class="text-sm text-green-700 dark:text-green-300">
                  <span class="font-medium">{{ settingsStore.enabledProviders.length }}</span> provider(s) enabled:
              ...
              <div class="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                <div class="flex items-center justify-between mb-3">
                  <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">기능별 필터</h3>
                  <button
                ...
                <p v-if="selectedTags.length > 0" class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  {{ filteredProviders.length }}개의 Provider가 선택된 태그와 일치합니다
                </p>
              </div>
              ...
              <div class="space-y-3">
                <div
                  ...
                  :class="[
                    ...
                    provider.enabled && (provider.apiKey || provider.isConnected)
                  ...
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                      ...
                      <div class="min-w-0">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 max-w-xs truncate">{{ provider.description }}</p>
                        <!-- Tags -->
                        <div class="flex flex-wrap gap-1 mt-2">
                          <span
                            v-for="tag in (provider.tags || []).slice(0, 5)"
                            :key="tag"
                            :class="['px-1.5 py-0.5 rounded text-[10px] font-medium', getTagColor(tag)]"
                          >
                            {{ settingsStore.getTagDisplayName(tag) }}
                          </span>
                          <span
                        ...
                        <div class="flex items-center gap-2 mt-1">
                          <span
                            v-if="provider.enabled && (provider.apiKey || provider.isConnected)"
                          ...
                          <span
                            v-else-if="provider.apiKey || provider.isConnected"
                          ...
                          <!-- Auth Method Badge -->
                          <span
                            v-if="provider.authMethods?.includes('oauth') || provider.authMethods?.includes('both')"
                          ...
                          <span v-if="provider.defaultModel" class="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[120px]">
          ...
          <div v-if="activeTab === 'shortcuts'" class="space-y-6">
            <section>
              ...
              <div class="space-y-4">
                <div
                  v-for="(shortcuts, category) in settingsStore.shortcutsByCategory"
                  :key="category"
                  ...
                  <div class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    >
                      ...
                      <div class="flex items-center gap-1">
                        <kbd
                          v-for="(key, index) in (shortcut.customKeys || shortcut.keys)"
                          :key="index"
          ...
          <div v-if="activeTab === 'profile'">
            <section>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">User Profile</h2>
          ...
          <div v-if="activeTab === 'data'" class="space-y-6">
            <section>
              ...
              <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm divide-y divide-gray-200 dark:divide-gray-700">
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Export Settings</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Download your settings as a JSON file</p>
                  </div>
                  <button
                ...
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Import Settings</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Load settings from a JSON file</p>
                  </div>
                  <button
                ...
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Reset to Defaults</h3>
            ...
            <section>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Setup Wizard</h2>
              <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm divide-y divide-gray-200 dark:divide-gray-700">
                <div class="flex items-center justify-between p-4">
                  <div>
                    <h3 class="font-medium text-gray-900 dark:text-white">Run Setup Wizard Again</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      {{ settingsStore.setupWizard.completed ? '초기 설정 위자드를 다시 실행합니다' : '초기 설정 위자드가 아직 완료되지 않았습니다' }}
                    </p>
                  </div>
                  <button
                ...
                <div v-if="settingsStore.setupWizard.completed || settingsStore.setupWizard.skipped" class="p-4">
                  <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span v-if="settingsStore.setupWizard.completed">
                      Setup completed on {{ new Date(settingsStore.setupWizard.completedAt || '').toLocaleDateString() }}
                    </span>
                    <span v-else-if="settingsStore.setupWizard.skipped">
                      Setup skipped on {{ new Date(settingsStore.setupWizard.skippedAt || '').toLocaleDateString() }}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
                <div class="space-y-3 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-gray-900 dark:text-white font-medium">HighAIFlow</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500 dark:text-gray-400">Version</span>
                    <span class="text-gray-900 dark:text-white">{{ appInfo?.version || '0.1.0' }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500 dark:text-gray-400">Platform</span>
                    <span class="text-gray-900 dark:text-white capitalize">{{ appInfo?.platform || 'unknown' }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500 dark:text-gray-400">Environment</span>
                    <span class="text-gray-900 dark:text-white">{{ appInfo?.isDev ? 'Development' : 'Production' }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500 dark:text-gray-400">Created by</span>
                    <span class="text-gray-900 dark:text-white font-medium">HighGarden</span>
                  </div>
                </div>
                <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <p class="text-xs text-gray-500 dark:text-gray-400">
                    HighAIFlow is an AI Workflow management tool that helps you automate and streamline your AI-powered workflows.
                  </p>
                </div>
              </div>
            </section>
