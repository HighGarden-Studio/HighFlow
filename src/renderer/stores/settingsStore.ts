...
function mergeFeatureScopeSelections(
  ...
  return Array.from(
    new Set(existing.filter((scope): scope is string => typeof scope === 'string' && scope.length > 0))
  );
}
...
export const useSettingsStore = defineStore('settings', () => {
  ...
  const aiProviders = ref<AIProviderConfig[]>([
    ...
    },
    {
      ...
      apiKey: '',
      enabled: false,
      models: ['meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo', 'meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo', 'mistralai/Mixtral-8x22B-Instruct-v0.1'],
    ...
    },
    {
      ...
      apiKey: '',
      enabled: false,
      models: ['accounts/fireworks/models/llama-v3p1-70b-instruct', 'accounts/fireworks/models/llama-v3p1-8b-instruct', 'accounts/fireworks/models/mixtral-8x22b-instruct'],
    ...
    // === Local / Self-hosted ===
    {
      ...
      enabled: false,
      models: ['llama3.1', 'llama3.1:70b', 'codellama', 'mistral', 'mixtral', 'qwen2.5-coder'],
    ...
    // === Specialized Providers ===
    {
      ...
      tags: ['chat', 'code', 'reasoning', 'image-analysis', 'multi-modal', 'agent', 'free-tier'],
      authMethods: ['apiKey', 'oauth'],
      apiKey: '',
      oauth: {
    ...
    },
    {
      ...
      apiKey: '',
      enabled: false,
      models: ['meta-llama/Meta-Llama-3.1-70B-Instruct', 'mistralai/Mixtral-8x7B-Instruct-v0.1', 'bigcode/starcoder2-15b'],
    ...
    },
    {
      ...
      apiKey: '',
      enabled: false,
      models: ['meta/meta-llama-3.1-405b-instruct', 'meta/llama-2-70b-chat', 'mistralai/mixtral-8x7b-instruct-v0.1'],
    ...
    // === Image/Video Generation Providers ===
    {
      ...
      apiKey: '',
      enabled: false,
      models: ['stable-diffusion-xl-1024-v1-0', 'stable-diffusion-v1-6', 'stable-video-diffusion', 'stable-image-core', 'stable-image-ultra'],
    ...
    // === Audio/TTS/Music Providers ===
    {
      ...
      apiKey: '',
      enabled: false,
      models: ['en-US-Neural2-A', 'en-US-Neural2-C', 'en-US-Wavenet-D', 'ko-KR-Neural2-A', 'ko-KR-Wavenet-A'],
  ...
  const mcpServers = ref<MCPServerConfig[]>(mcpServerSeeds.map((seed) => withPermissionDefaults(seed)));
  ...
  const defaultProvider = computed(() =>
    enabledProviders.value[0] || null
  );

  const connectedIntegrations = computed(() =>
    integrations.value.filter((i) => i.connected)
  );
  ...
  function getMCPServersByTags(tags: MCPServerTag[]): MCPServerConfig[] {
    ...
    return mcpServers.value.filter(server =>
      server.tags && server.tags.some(tag => tags.includes(tag))
    );
  }
  ...
  function getProvidersByAllTags(tags: AIProviderTag[]): AIProviderConfig[] {
    if (tags.length === 0) return aiProviders.value;
    return aiProviders.value.filter(provider =>
      provider.tags && tags.every(tag => provider.tags.includes(tag))
    );
  }
  ...
  function cloneProviderConfig(provider: AIProviderConfig): AIProviderConfig {
    return {
      ...
      oauth: provider.oauth ? { ...provider.oauth, scopes: provider.oauth.scopes ? [...provider.oauth.scopes] : undefined } : undefined,
    };
  }
    ...
    try {
      ...
      if (storedProviders) {
        ...
        const mergedProviders = defaultProviders.map(defaultProvider => {
          const storedProvider = storedProviders.find(sp => sp.id === defaultProvider.id);
          if (storedProvider) {
            // Check if stored defaultModel is still valid (exists in models list)
            const storedModelValid = storedProvider.defaultModel &&
            ...
            return {
              ...
              defaultModel: storedModelValid ? storedProvider.defaultModel : defaultProvider.defaultModel,
      ...
      const storedGeneral = loadFromStorage<Partial<GeneralSettings> | null>('generalSettings', null);
      if (storedGeneral) {
        generalSettings.value = { ...generalSettings.value, ...storedGeneral };
      }

      const storedShortcuts = loadFromStorage<KeyboardShortcut[] | null>('keyboardShortcuts', null);
      if (storedShortcuts) {
        keyboardShortcuts.value = storedShortcuts;
      }

      const storedIntegrations = loadFromStorage<IntegrationConfig[] | null>('integrations', null);
        ...
        const mergedServers = defaultServers.map(defaultServer => {
          ...
          if (storedServer) {
            return withPermissionDefaults({
              ...
              lastInstalledAt: storedServer.lastInstalledAt ?? defaultServer.lastInstalledAt,
      ...
      if (currentServer) {
        const nextServer = {
          ...
          scopes: updates.scopes
            ? Array.from(new Set(updates.scopes.filter((scope) => typeof scope === 'string')))
