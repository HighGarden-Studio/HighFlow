...
export interface ProjectsAPI {
  ...
  create: (data: { title: string; description?: string; ownerId: number; baseDevFolder?: string | null; projectGuidelines?: string | null }) => Promise<Project>;
  update: (id: number, data: Partial<{ title: string; description: string; status: string; baseDevFolder?: string | null; projectGuidelines?: string | null }>) => Promise<Project>;
...
export interface TasksAPI {
  ...
  update: (id: number, data: Partial<{ title: string; description: string; status: string; priority: string }>) => Promise<Task>;
  delete: (id: number) => Promise<void>;
...
export interface WorkflowAPI {
  ...
  list: (projectId: number, filters?: { status?: WorkflowStatus; limit?: number }) => Promise<unknown[]>;
  listActive: (projectId?: number) => Promise<unknown[]>;
  updateStatus: (workflowId: string, status: WorkflowStatus, additionalData?: unknown) => Promise<unknown>;
  updateProgress: (workflowId: string, progress: {
...
export interface LocalAgentsAPI {
  ...
  onSessionResponse: (callback: (sessionId: string, response: AgentResponse) => void) => () => void;
...
export interface TaskExecutionAPI {
  ...
  // Approval flow
  requestApproval: (taskId: number, data: ApprovalRequest) => Promise<{ success: boolean; error?: string }>;
  ...
  requestChanges: (taskId: number, refinementPrompt: string) => Promise<{ success: boolean; error?: string }>;
  requestAdditionalWork: (taskId: number, additionalWorkPrompt: string) => Promise<{ success: boolean; error?: string }>;
  ...
  // Progress updates
  updateProgress: (taskId: number, progress: ExecutionProgress) => Promise<{ success: boolean; error?: string }>;
  ...
  onCompleted: (callback: (data: { taskId: number; result: ExecutionResult }) => void) => () => void;
  ...
  onApprovalRequired: (callback: (data: { taskId: number } & ApprovalRequest) => void) => () => void;
  ...
  onChangesRequested: (callback: (data: { taskId: number; refinementPrompt: string }) => void) => () => void;
  onAdditionalWorkRequested: (callback: (data: { taskId: number; additionalWorkPrompt: string }) => void) => () => void;
  ...
  onReviewProgress: (callback: (data: { taskId: number; progress?: number; phase?: string; content?: string }) => void) => () => void;
  onAutoReviewCompleted: (callback: (data: { taskId: number; result: unknown; passed: boolean; score: number }) => void) => () => void;
...
export interface TaskHistoryAPI {
  ...
  add: (taskId: number, eventType: string, eventData?: unknown, metadata?: unknown) => Promise<unknown>;
}
