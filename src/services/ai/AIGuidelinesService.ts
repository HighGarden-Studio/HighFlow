...
class AIGuidelinesService {
     ...
     */
    generateGuidelines(concretizedIdea: ConcretizedIdea, tasks: SuggestedTask[]): string {
     ...
     * 지침 데이터 구조 생성
     */
    private buildGuidelines(idea: ConcretizedIdea, tasks: SuggestedTask[]): ProjectGuidelines {
        // 기술 스택 추출
     ...
     * 아키텍처 설명 생성
     */
    private generateArchitectureDescription(_idea: ConcretizedIdea, stack: string[]): string {
        const parts: string[] = [];
    ...
    private generateTaskGuidelines(tasks: SuggestedTask[]): string {
        ...
        categories.forEach((categoryTasks, category) => {
            guidelines.push(
                `### ${this.formatCategoryName(category)} (${categoryTasks.length}개 태스크)`
            );
     ...
     */
    updateGuidelinesWithNewTasks(existingGuidelines: string, newTasks: SuggestedTask[]): string {
        // 새 태스크 섹션 생성
