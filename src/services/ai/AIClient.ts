...
class AIClient {
  private getApiKey(provider: AIProviderType): string | null {
    ...
    try {
      ...
      if (providerConfig?.apiKey) {
        ...
        console.log(`[AIClient] Got API key for ${provider} from Pinia store: ${maskedKey} (length: ${key.length})`);
        return key;
      }
    } catch (e) {
    ...
    try {
      ...
      if (stored) {
        ...
        if (providerConfig?.apiKey) {
          const key = providerConfig.apiKey;
          const maskedKey = key.length > 10 ? `${key.substring(0, 8)}...${key.substring(key.length - 4)}` : '***';
          console.log(`[AIClient] Got API key for ${provider} from localStorage: ${maskedKey} (length: ${key.length})`);
          return key;
        }
      }
    } catch (e) {
  ...
  async googleCompletion(
    ...
    const apiVersion = model.includes('exp') || model.includes('thinking') ? 'v1beta' : 'v1beta';
  ...
  async completion(
    messages: AIMessage[],
    options: AICompletionOptions & { provider?: AIProviderType; preferredProvider?: AIProviderType } = {}
  ): Promise<AICompletionResponse> {
