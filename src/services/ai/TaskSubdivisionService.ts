...
export class TaskSubdivisionService {
    ...
    private analyzeTask(task: Task): {
        ...
        // 제안할 서브태스크 수
        const suggestedCount = complexity === 'high' ? 5 : complexity === 'medium' ? 3 : 2;
    ...
    private generateSuggestions(
        ...
        for (let i = 0; i < analysis.suggestedCount; i++) {
            ...
            subtasks.push({
                ...
                priority:
                    i === 0 ? 'high' : i === analysis.suggestedCount - 1 ? 'medium' : 'medium',
        ...
        return {
            ...
            totalEstimatedMinutes: subtasks.reduce(
    ...
    private generateReasoning(task: Task, analysis: any, subtaskCount: number): string {
        ...
        return (
            `이 작업은 예상 소요 시간이 ${estimatedHours}시간이고 복잡도가 "${complexityText}"로 평가됩니다. ` +
