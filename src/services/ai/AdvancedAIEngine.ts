...
Respond in JSON format.`;
  ...
  private parseAnalysisResponse(content: string): Omit<PromptAnalysisResult, 'suggestedTemplates'> {
    try {
  ...
  private parseDecompositionResponse(content: string): TaskDecompositionResult {
    try {
      ...
      return {
        ...
        executionPlan: data.executionPlan || { phases: [], criticalPath: [], parallelizableGroups: [], estimatedTotalTime: 0, bottlenecks: [] },
      ...
      return {
        ...
        executionPlan: { phases: [], criticalPath: [], parallelizableGroups: [], estimatedTotalTime: 0, bottlenecks: [] },
        estimatedTime: 0,
        estimatedCost: 0,
        risks: [],
      };
  ...
  private parseSkillsResponse(content: string, availableSkills: Skill[]): SkillRecommendation[] {
    try {
      ...
      return (data.recommendations || data || []).map((rec: any) => {
        const skill = availableSkills.find((s) => s.id === rec.skillId || s.name === rec.skillName);
        return {
  ...
  private parseMCPMatchResponse(content: string, availableMCPs: MCPIntegration[]): MCPMatch[] {
    try {
      ...
      return (data.matches || data || []).map((match: any) => {
        const mcp = availableMCPs.find((m) => m.id === match.mcpId || m.name === match.mcpName);
        return {
  ...
  private extractRequirements(content: string): RequirementGap[] {
    ...
    if (content.toLowerCase().includes('deadline') || content.toLowerCase().includes('timeline')) {
      requirements.push({
        category: 'timeline',
  ...
  private async fetchMatchingTemplates(_domain: string, _keywords: string[]): Promise<Template[]> {
