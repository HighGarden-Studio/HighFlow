...
- 한국어로 작성`;
    ...
    private async generateFollowUpResponseWithAI(
        ...
        try {
            ...
            const systemPrompt = `당신은 사용자의 아이디어를 깊이 이해하고 구체화하는 전문 인터뷰어입니다.

## 핵심 목표
사용자의 고유한 아이디어, 독창적인 부분, 차별점을 최대한 파악하여 기록하는 것이 목표입니다.
일반적인 질문보다 사용자가 말한 내용을 기반으로 더 깊이 있는 질문을 해주세요.
...
이 경우 다음과 같이 **구체적인 예시와 제안을 함께 제시**해주세요:

1. 사용자의 아이디어("${context.originalIdea}")와 유사한 프로젝트들에서 일반적으로 필요한 기능들을 3-5개 예시로 제안
2. 각 예시에 대해 간단한 설명 추가
3. "이 중에서 원하시는 것이 있으신가요? 또는 다른 생각이 있으시면 말씀해주세요." 형태로 마무리

예시 형식:
"비슷한 프로젝트들에서는 보통 이런 기능들이 들어가요:
• **사용자 인증**: 로그인/회원가입 기능
• **대시보드**: 주요 정보를 한눈에 보는 화면
• **알림 기능**: 중요한 이벤트 알림
• **데이터 내보내기**: CSV나 PDF로 내보내기
...
2. **태스크 도출 (가장 중요)**:
   ...
   - 각 태스크는 AI가 **30~90분 내에 수행 가능한 단위**로 최대한 잘게 쪼개세요. 규모가 크면 **서브테스크로 분리**하여 executionOrder와 dependencies로 연결하세요.
   - "사용자가 ~라고 함" 같은 표현을 쓰지 말고, "로그인 페이지 구현", "API 스키마 설계" 등 명확한 작업 지시로 작성하세요.

## 태스크 개수 가이드라인 (매우 중요!)
프로젝트 규모에 따라 충분한 수의 태스크를 생성해야 합니다:
- **간단한 스크립트/유틸리티**: 3-5개 태스크
- **소규모 프로젝트 (랜딩 페이지, 간단한 API 등)**: 6-10개 태스크
- **중간 규모 앱 (풀스택 웹앱, 모바일 앱, 데스크톱 앱)**: 12-18개 태스크
- **대규모 프로젝트 (복잡한 시스템, 멀티모듈)**: 18-25개 태스크

예를 들어, Electron 기반 워크플로우 매니저 앱이라면 최소 15개 이상의 태스크가 필요합니다.
각 기능을 세분화하여 AI가 각 태스크를 독립적으로 수행할 수 있도록 분할하세요.

## 태스크 생성 규칙
- 각 태스크는 사용자 아이디어의 특정 기능이나 요소를 직접 구현해야 합니다
- 태스크 제목은 구체적인 기능명을 포함해야 합니다 (예: "사용자 인증 시스템 구현" → "OAuth 기반 소셜 로그인 구현")
- 태스크 설명에는 구현할 내용의 세부사항을 포함합니다
- 일반적인 "설계", "테스트", "문서화" 태스크는 최소화하고, 핵심 기능 구현 태스크를 우선합니다
- 사용자가 언급한 특별한 요구사항이나 제약조건을 태스크에 반영합니다
- **큰 기능은 여러 태스크로 분할**: 예를 들어 "대시보드 구현"이 아니라 "대시보드 레이아웃 구현", "대시보드 차트 컴포넌트 구현", "대시보드 데이터 연동" 등으로 분할

## 태스크 우선순위
1. 사용자가 가장 중요하다고 언급한 핵심 기능
2. 아이디어의 독창적/차별화된 기능
3. 기본 인프라 및 기반 작업
4. 부가 기능 및 개선사항
...
- text: 일반 텍스트 (설명, 분석 결과 등)
- markdown: 마크다운 문서 (README, 가이드 등)
- html: HTML 코드 (웹 페이지, 이메일 템플릿 등)
- pdf: PDF 문서 생성 지시
- json: JSON 데이터 (API 응답, 설정 파일 등)
- yaml: YAML 설정 파일 (config, docker-compose 등)
- csv: CSV 데이터 (데이터 내보내기 등)
- sql: SQL 쿼리/스키마 (데이터베이스 관련)
- shell: 셸 스크립트 (bash, zsh 등)
- mermaid: Mermaid 다이어그램 (플로우차트, 시퀀스 다이어그램 등)
- svg: SVG 이미지 (아이콘, 다이어그램 등)
- png: 이미지 생성 지시 (이미지 AI 필요)
- mp4: 비디오 생성 지시 (비디오 AI 필요)
- mp3: 오디오 생성 지시 (오디오 AI 필요)
- diff: 코드 변경사항 (git diff 형식)
- log: 로그 형식 출력
- code: 소스 코드 (codeLanguage 필드로 언어 지정)
...
- **품질 기준**: 코드 스타일, 주석, 테스트 가능성 등
- **컨텍스트 정보**: 이 태스크가 전체 프로젝트에서 어떤 역할을 하는지
- **결과물 형식 지시**: "결과물은 [형식]으로 작성해주세요"

**좋은 promptTemplate 예시:**
\`\`\`
Vue 3 Composition API를 사용하여 칸반 보드의 TaskCard 컴포넌트를 구현하세요.

## 요구사항
- 태스크 제목, 설명, 상태, 우선순위를 표시
- 드래그 앤 드롭을 위한 draggable 속성 지원
- 상태별 색상 배지 (TODO: 회색, IN_PROGRESS: 파란색, DONE: 초록색)
- 클릭 시 상세 모달 열기 이벤트 emit
...
## 결과물
TypeScript 코드로 작성하고, 각 prop과 emit에 타입을 명시해주세요.
\`\`\`
