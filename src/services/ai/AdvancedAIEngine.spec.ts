...
import type { UserContext, ConversationContext, Answer, StructuredRequirement } from '@core/types/ai';
import type { Task, Skill, MCPIntegration } from '@core/types/database';
...
vi.mock('./providers/ProviderFactory', () => ({
  ProviderFactory: vi.fn().mockImplementation(() => ({
    getProvider: vi.fn().mockResolvedValue({
      ...
      getSupportedFeatures: vi.fn().mockReturnValue(['json', 'streaming']),
    ...
    getAllProviders: vi.fn().mockResolvedValue([
      ...
      },
      {
        name: 'openai',
        models: ['gpt-4-turbo'],
        getSupportedFeatures: vi.fn().mockReturnValue(['json', 'functions']),
...
const mockUserContext: UserContext = {
  ...
  recentProjects: [
    { title: 'E-commerce Site', id: 'proj-1' },
    { title: 'Blog Platform', id: 'proj-2' },
  ],
  preferences: {
...
const mockAnswers: Answer[] = [
  { questionId: 'q1', value: '사진 공유 기능' },
  { questionId: 'q2', value: '웹과 모바일' },
  { questionId: 'q3', value: '소셜 로그인' },
];
...
describe('AdvancedAIEngine', () => {
    ...
    it('should analyze prompt and return requirements', async () => {
      mockExecute.mockResolvedValue({
        content: JSON.stringify({
          requirements: [
            { category: 'feature', question: '어떤 기능이 필요하나요?', importance: 'high' },
          ],
          complexity: 'complex',
  ...
  describe('Conversation Management', () => {
    it('should create a new conversation context', () => {
      const context = engine.createConversation(
        '새 프로젝트 만들기',
        1
      );
