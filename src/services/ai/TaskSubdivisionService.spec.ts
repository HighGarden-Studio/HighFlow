...
const createMockTask = (overrides: Partial<Task> = {}): Task =>
    ({
        // id: 1, // Task has no global ID
        projectId: 1,
        projectSequence: 1,
        title: 'Implement User Authentication',
        description:
            'Create a complete user authentication system with login, registration, and password reset',
        status: 'todo',
        priority: 'high',
...
describe('TaskSubdivisionService', () => {
    ...
    describe('suggestSubdivision', () => {
        ...
        it('should generate more subtasks for complex tasks', async () => {
            const complexTask = createMockTask({
                ...
                description:
                    'Develop a full-featured e-commerce platform including user management, product catalog, shopping cart, checkout process, payment gateway integration, and inventory management system',
    ...
    describe('Pattern-based Subdivision', () => {
        ...
        it('should use API patterns for API-related tasks', async () => {
            ...
            expect(
                titles.some(
                    (t) => t.includes('api') || t.includes('endpoint') || t.includes('엔드포인트')
                )
            ).toBe(true);
        });
