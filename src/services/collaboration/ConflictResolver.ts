...
export class ConflictResolver {
  ...
  public resolveOfflineConflicts(
    localChanges: Array<{ type: string; entityId: number; field?: string; value: any; timestamp: Date; userId: number }>,
    serverState: Map<string, { value: any; timestamp: Date; userId: number }>
  ): ConflictResolution[] {
    const resolutions: ConflictResolution[] = [];

    localChanges.forEach(localChange => {
      const key = this.generateConflictKey(localChange.type, localChange.entityId, localChange.field);
  ...
  public getStatistics() {
    return {
      ...
      resolutionsByType: this.resolutionHistory.reduce((acc, res) => {
