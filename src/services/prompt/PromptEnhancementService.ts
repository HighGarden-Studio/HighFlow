...
class PromptEnhancementService {
    ...
    private initializePatterns(): Map<string, RegExp> {
        return new Map([
            [
                'vague_words',
                /\b(것|무언가|어떤|좀|약간|대충|그냥|뭔가|something|some|kind of|sort of|maybe|probably)\b/gi,
            ],
            [
                'action_verbs',
                /\b(만들어|작성|생성|분석|검토|수정|개선|구현|설계|create|write|build|analyze|review|implement|design)\b/gi,
            ],
            [
                'output_format',
                /\b(형식|포맷|JSON|XML|HTML|마크다운|format|output|return|response)\b/gi,
            ],
            [
                'context_markers',
                /\b(배경|컨텍스트|상황|목적|이유|context|background|purpose|reason|because)\b/gi,
            ],
            [
                'constraints',
                /\b(제한|조건|요구사항|필수|반드시|constraint|requirement|must|should|limit)\b/gi,
            ],
    ...
    private measureClarity(prompt: string): number {
        ...
        const punctuationRatio =
            (prompt.match(/[,，.。:：;；]/g) || []).length / Math.max(prompt.length / 50, 1);
    ...
    private measureSpecificity(prompt: string): number {
        ...
        const technicalPatterns =
            /\b(API|함수|클래스|컴포넌트|데이터베이스|function|class|component|database|module|service)\b/gi;
    ...
    private measureActionability(prompt: string): number {
        ...
        const imperativePatterns =
            /^(해주세요|작성해|만들어|분석해|Please|Create|Write|Build|Analyze|Generate)/gim;
        ...
        if (scores.completeness < 50) {
            ...
            if (!contextMatches) {
                issues.push({
                    ...
                    severity: 'medium',
                    message:
                        '배경 정보나 컨텍스트가 부족합니다. 왜 이 작업이 필요한지 설명해주세요.',
                });
            }
        }
    ...
    private generateSuggestions(_prompt: string, issues: PromptIssue[]): PromptSuggestion[] {
    ...
    private hasStructure(prompt: string): boolean {
        const structurePatterns =
            /^(#|##|\d+\.|•|-|\*|목표|배경|요구사항|Goal|Background|Requirements)/m;
    ...
    private getOutputFormatDescription(format: string): string {
        ...
        const result = formats[format as keyof typeof formats];
        return (typeof result === 'string' ? result : formats.general) as string;
    }
    ...
    private ensureActionOriented(prompt: string): string {
        ...
        const actionStart =
            /^(만들어|작성|생성|분석|검토|수정|개선|구현|설계|해주세요|Please|Create|Write|Build|Analyze|Generate|Design|Implement)/i;
