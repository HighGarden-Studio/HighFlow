...
class PromptVersionManager {
  ...
  getStats(taskId: number): {
    ...
    const avgScore = versionsWithScore.length > 0
      ? versionsWithScore.reduce((sum, v) => sum + v.analysis!.overallScore, 0) / versionsWithScore.length
      : undefined;

    // Calculate score improvement
    let scoreImprovement: number | undefined;
    if (firstVersion.analysis?.overallScore && lastVersion.analysis?.overallScore) {
      scoreImprovement = lastVersion.analysis.overallScore - firstVersion.analysis.overallScore;
    }
    ...
    const mostUsedSource = [...sourceCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || 'manual';
