...
- 사용 예시 제공`,
        variables: [
            ...
            {
                name: 'requirements',
                description: '구체적인 요구사항',
                type: 'multiline',
                required: true,
            },
            {
                name: 'codeStyle',
                description: '코드 스타일 가이드',
                type: 'text',
                required: false,
...
4. 리팩토링된 코드 예시`,
        variables: [
            ...
            {
                name: 'reviewFocus',
                description: '특별히 집중할 부분',
                type: 'multiline',
                required: false,
                defaultValue: '전반적인 코드 품질',
            },
        ],
        examples: ['API 엔드포인트 보안 검토', '성능 최적화 관점 리뷰', '신규 개발자 코드 멘토링'],
        tags: ['리뷰', '품질', '개선'],
    },
    {
        name: '버그 디버깅',
...
4. 재발 방지를 위한 제안`,
        variables: [
            ...
            {
                name: 'actualBehavior',
                description: '실제 발생한 동작',
                type: 'text',
                required: true,
            },
        ],
        examples: ['API 호출 시 500 에러 발생', '메모리 누수 문제 해결', '비동기 처리 관련 버그'],
        tags: ['디버깅', '버그', '수정'],
    },
...
- 다이어그램 설명 포함 (필요시)`,
        variables: [
            ...
            },
            {
                ...
                type: 'select',
                required: false,
                options: ['공식적', '친근한', '기술적', '교육적'],
                defaultValue: '기술적',
            },
        ],
        examples: ['REST API 엔드포인트 문서화', '신규 기능 사용자 가이드', '시스템 아키텍처 설명'],
        tags: ['문서', '기술문서', '가이드'],
    },
...
4. 마이그레이션 스크립트 (선택)`,
        variables: [
            ...
            {
                name: 'entities',
                description: '주요 엔티티 나열',
                type: 'multiline',
                required: true,
            },
            {
                name: 'relationships',
                description: '엔티티 간 관계',
                type: 'multiline',
                required: false,
            },
            {
            ...
            {
                name: 'scalabilityNotes',
                description: '확장성 고려사항',
                type: 'text',
                required: false,
                defaultValue: '해당 없음',
            },
        ],
        examples: ['사용자 관리 시스템 스키마', '주문 처리 데이터 모델', '콘텐츠 관리 시스템 설계'],
        tags: ['데이터', '스키마', '설계'],
    },
...
4. 사용 예시`,
        variables: [
            ...
            },
            {
                ...
                description: '반응형 요구사항',
                type: 'text',
                required: false,
                defaultValue: '모바일 퍼스트',
            },
            {
                name: 'accessibilityNotes',
                description: '접근성 고려사항',
                type: 'text',
                required: false,
                defaultValue: 'WCAG 2.1 AA',
            },
        ],
        examples: ['데이터 테이블 컴포넌트', '모달 다이얼로그', '폼 인풋 컴포넌트 세트'],
        tags: ['디자인', 'UI', '컴포넌트'],
    },
];
...
class PromptTemplateLibrary {
    ...
    private saveToStorage(): void {
        try {
            const customTemplates = [...this.templates.values()].filter(
                (t) => !t.id.startsWith('builtin_')
            );
     ...
     */
    createTemplate(
        template: Omit<PromptTemplate, 'id' | 'usageCount' | 'rating' | 'createdAt' | 'updatedAt'>
    ): PromptTemplate {
     ...
     */
    renderTemplate(templateId: string, variables: Record<string, string>): TemplateRenderResult {
    ...
    validateTemplate(template: string, variables: TemplateVariable[]): TemplateValidation {
        ...
        while ((match = variablePattern.exec(template)) !== null) {
            ...
            if (varName && !definedVarNames.has(varName)) {
