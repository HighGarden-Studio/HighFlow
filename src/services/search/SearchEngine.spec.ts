...
describe('SearchEngine', () => {
    ...
    beforeEach(async () => {
        ...
        for (const doc of mockDocuments) {
            await engine.indexEntity(doc.entityType, doc.entityId, doc.content, doc.metadata);
        }
    });
    ...
    describe('Search with Filters', () => {
        ...
        it('should filter by project ID', async () => {
            ...
            results.forEach((result) => {
                expect((result.metadata as any).projectId).toBe(1);
            });
        });
        ...
        it('should filter by tags', async () => {
            ...
            results.forEach((result) => {
                const tags = (result.metadata as any).tags as string[];
        ...
        it('should filter by status', async () => {
            ...
            results.forEach((result) => {
                expect((result.metadata as any).status).toBe('todo');
            });
        });
    ...
    describe('Saved Searches', () => {
        ...
        it('should throw error for non-existent saved search', async () => {
            await expect(engine.executeSavedSearch('NonExistent')).rejects.toThrow(
                'Saved search "NonExistent" not found'
            );
        });
