...
export class AutomationEngine {
    ...
    private async actionUpdateTask(
        action: Extract<Action, { type: 'task.update' }>,
        _context: Event
    ): Promise<any> {
     ...
     * Task 실행 액션
     */
    private async actionExecuteTask(
        action: Extract<Action, { type: 'task.execute' }>,
        _context: Event
    ): Promise<any> {
        ...
        if (typeof window !== 'undefined' && window.electron?.tasks) {
            ...
            // Fetch project to get MCP config and AI settings
            let projectMcpConfig = null;
            let project = null;
            try {
                if (window.electron?.projects) {
                    project = await window.electron.projects.get(task.projectId);
                ...
                console.warn(
                    `[AutomationEngine] Failed to fetch project config for task ${task.id}:`,
                    error
                );
            }
            ...
            const executionContext: ExecutionContext = {
                ...
                metadata: {
                    ...
                    ruleId: action.type,
                    projectMcpConfig,
                    project, // Inject full project for AI settings inheritance
                },
            };
    ...
    private async actionSendNotification(
        action: Extract<Action, { type: 'notification.send' }>,
        _context: Event
    ): Promise<any> {
        console.log(
            `[AutomationEngine] Sending notification to user ${action.userId} via ${action.channel}:`,
            action.message
        );
    ...
    private async actionCallWebhook(
        action: Extract<Action, { type: 'webhook.call' }>,
        _context: Event
    ): Promise<any> {
     ...
     * AI 실행 액션
     */
    private async actionExecuteAI(
        action: Extract<Action, { type: 'ai.execute' }>,
        _context: Event
    ): Promise<any> {
        console.log(
            `[AutomationEngine] Executing AI task ${action.taskId} with provider ${action.provider || 'default'}`
        );
        ...
        if (typeof window !== 'undefined' && window.electron?.tasks) {
            ...
            // Fetch project to get MCP config
            let projectMcpConfig = null;
            let project = null; // Declare project here
            try {
                if (window.electron?.projects) {
                    project = await window.electron.projects.get(task.projectId);
                    if (project?.mcpConfig) {
     ...
     * Slack 통합 액션
     */
    private async actionSlackIntegration(
        action: Extract<Action, { type: 'integration.slack' }>,
        _context: Event
    ): Promise<any> {
        console.log(
            `[AutomationEngine] Sending Slack message to ${action.channel}:`,
            action.message
        );
     ...
     * 워크플로우 시작 액션
     */
    private async actionStartWorkflow(
        action: Extract<Action, { type: 'workflow.start' }>,
        _context: Event
    ): Promise<any> {
        ...
        if (typeof window !== 'undefined' && window.electron?.workflow) {
            ...
            const workflowStatus = (existingWorkflow as { status?: string } | null | undefined)
                ?.status;
     ...
     * 워크플로우 중지 액션
     */
    private async actionStopWorkflow(
        action: Extract<Action, { type: 'workflow.stop' }>,
        _context: Event
    ): Promise<any> {
     ...
     * 변수 설정 액션
     */
    private async actionSetVariable(
        action: Extract<Action, { type: 'variable.set' }>,
        _context: Event
    ): Promise<any> {
    ...
    async handleEvent(event: Event): Promise<void> {
        ...
        // 모든 매칭되는 규칙 실행
        const executions = Array.from(ruleIds).map((ruleId) => this.executeTrigger(ruleId, event));
    ...
    getRulesByProject(projectId: number): AutomationRule[] {
        return Array.from(this.rules.values()).filter(
            (rule) => rule.projectId === projectId || rule.projectId === undefined
        );
    }
    ...
    getStatistics(): {
        ...
        const rulesByTriggerType = rules.reduce(
            (acc, rule) => {
