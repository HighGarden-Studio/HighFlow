...
class ResultVersioningService {
  ...
  getVersionsByLabel(taskId: number, label: string): Version[] {
    ...
    return versionIds
      .map(id => this.getVersion(id))
      .filter((v): v is Version => v !== undefined && v.metadata.labels?.includes(label));
  }
  ...
  private createDelta(baseContent: string, newContent: string): string {
    ...
    const operations: { op: 'keep' | 'add' | 'del'; start: number; end?: number; text?: string }[] = [];
...
export function useResultVersioning() {
  return {
    ...
    getVersion: (versionId: string) =>
      resultVersioningService.getVersion(versionId),
    getLatestVersion: (taskId: number) =>
      resultVersioningService.getLatestVersion(taskId),
    getVersionHistory: (taskId: number) =>
      resultVersioningService.getVersionHistory(taskId),
    ...
    deleteVersion: (versionId: string) =>
      resultVersioningService.deleteVersion(versionId),
