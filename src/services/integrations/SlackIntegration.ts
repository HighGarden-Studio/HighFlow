...
export class SlackIntegration {
  ...
  async getChannels(includePrivate = false): Promise<SlackChannel[]> {
    ...
    do {
      ...
      const responseChannels = response.channels as Array<Record<string, unknown>> | undefined;
   ...
   */
  async sendDirectMessage(userId: string, message: Omit<SlackMessage, 'channel'>): Promise<SlackMessageResult> {
        ...
        users.push(
          ...members
            ...
            .map((u: Record<string, unknown>) => ({
              ...
              email: (u.profile as Record<string, unknown>)?.email as string | undefined,
