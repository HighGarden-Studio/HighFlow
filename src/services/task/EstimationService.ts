...
const COMPLEXITY_INDICATORS = {
  high: [
    'integration', 'architecture', 'security', 'migration',
    'authentication', 'oauth', 'database', 'real-time',
    'websocket', 'performance', 'optimization', 'scale',
    'distributed', 'microservice', 'api', 'graphql',
    '통합', '아키텍처', '보안', '마이그레이션', '인증', '최적화',
  ],
  medium: [
    'component', 'feature', 'refactor', 'test', 'ui',
    'form', 'validation', 'crud', 'styling', 'responsive',
    '컴포넌트', '기능', '리팩토링', '테스트', '화면',
  ],
  low: [
    'fix', 'bug', 'typo', 'update', 'docs', 'readme',
    'comment', 'cleanup', 'style', 'format',
    '수정', '버그', '문서', '정리', '스타일',
  ],
};
...
export class EstimationService {
  ...
  estimateTime(
    ...
    if (complexity.level === 'simple') {
      breakdown.push(
        { phase: '분석', minutes: Math.round(baseMinutes * 0.1), description: '요구사항 확인' },
        { phase: '구현', minutes: Math.round(baseMinutes * 0.7), description: '기능 구현' },
        { phase: '테스트', minutes: Math.round(baseMinutes * 0.2), description: '기본 테스트' }
      );
    } else if (complexity.level === 'moderate') {
      breakdown.push(
        { phase: '분석', minutes: Math.round(baseMinutes * 0.15), description: '요구사항 분석 및 설계' },
        { phase: '구현', minutes: Math.round(baseMinutes * 0.6), description: '기능 구현' },
        { phase: '테스트', minutes: Math.round(baseMinutes * 0.15), description: '테스트 및 검증' },
        { phase: '리뷰', minutes: Math.round(baseMinutes * 0.1), description: '코드 리뷰' }
      );
    } else if (complexity.level === 'complex') {
      breakdown.push(
        { phase: '분석/설계', minutes: Math.round(baseMinutes * 0.2), description: '상세 요구사항 분석 및 설계' },
        { phase: '구현', minutes: Math.round(baseMinutes * 0.5), description: '기능 구현' },
        { phase: '테스트', minutes: Math.round(baseMinutes * 0.2), description: '테스트 작성 및 실행' },
        { phase: '리뷰/QA', minutes: Math.round(baseMinutes * 0.1), description: '코드 리뷰 및 QA' }
      );
    } else {
      breakdown.push(
        { phase: '분석/설계', minutes: Math.round(baseMinutes * 0.25), description: '상세 분석, 아키텍처 설계' },
        { phase: '프로토타입', minutes: Math.round(baseMinutes * 0.15), description: '프로토타입 개발' },
  ...
  estimateCost(
    ...
    if (providerCosts) {
      const modelCosts = providerCosts[modelName as keyof typeof providerCosts] ||
  ...
  private generateTimeReasoning(
    ...
    return `이 작업은 "${levelText}" 수준으로 평가되어 약 ${hours}시간이 소요될 것으로 예상됩니다. ` +
  ...
  private generateCostReasoning(
    ...
    return `${provider}의 ${model} 모델 사용 시 예상 비용은 약 $${totalCost.toFixed(2)}입니다. ` +
  ...
  private generateSuggestions(
    ...
    if (cost.estimatedCost > 1) {
      suggestions.push('비용 절감을 위해 더 저렴한 모델 (Claude Haiku, GPT-3.5) 고려해보세요.');
    }
