...
`;
  ...
  private getRelevantResults(
    ...
    for (const depId of dependencies) {
      const result = allResults.find(r => r.taskId === depId) || this.resultCache.get(depId);
  ...
  private applyTemplate(results: FormattedResult[], template: string): string {
    ...
    if (perResultMatch) {
      ...
      output = output.replace(/\{\{#each\}\}[\s\S]*?\{\{\/each\}\}/, perResultOutputs.join('\n'));
    }
...
export function useContextPropagation() {
  return {
    ...
    getPropagatedContext: (taskId: number, allResults: TaskResult[], taskTitles: Map<number, string>) =>
