...
export class ExecutionPlanner {
  ...
  async createPlan(tasks: Task[]): Promise<ExecutionPlan> {
    ...
    const stages = this.groupIntoStages(sortedNodes, dependencyGraph);
  ...
  private buildDependencyGraph(tasks: Task[]): DependencyGraph {
    ...
    for (const task of tasks) {
      nodes.push({
        ...
        task,
        level: 0, // 나중에 계산
        estimatedDuration: task.estimatedMinutes ? task.estimatedMinutes * 60 * 1000 : 300000, // 기본 5분
      });
    }
   ...
   */
  private groupIntoStages(sortedNodes: GraphNode[], graph: DependencyGraph): Stage[] {
    const stages: Stage[] = [];
  ...
  async optimizePlan(plan: ExecutionPlan, constraints: Constraints): Promise<ExecutionPlan> {
    ...
    if (constraints.maxParallelism) {
      optimizedPlan = this.applyParallelismConstraint(optimizedPlan, constraints.maxParallelism);
    }

    // 2. 비용 제한 적용
    if (constraints.maxCost && plan.estimatedCost > constraints.maxCost) {
      throw new Error(`Estimated cost ${plan.estimatedCost} exceeds budget ${constraints.maxCost}`);
    }
   ...
   * 시간 최적화
   */
  private optimizeForTime(plan: ExecutionPlan, maxDuration: number): ExecutionPlan {
    // Critical Path 상의 Task들을 우선 처리
    const criticalTaskIds = new Set(plan.criticalPath.map(t => t.id));

    const reorderedStages = plan.stages.map(stage => {
      const sortedTasks = stage.tasks.sort((a, b) => {
        const aIsCritical = criticalTaskIds.has(a.id);
        const bIsCritical = criticalTaskIds.has(b.id);
   ...
   * 우선순위 재정렬
   */
  private reorderByPriority(plan: ExecutionPlan, priority: string): ExecutionPlan {
    // 높은 우선순위 Task들을 앞으로
    const reorderedStages = plan.stages.map(stage => {
      const sortedTasks = [...stage.tasks].sort((a, b) => {
        const priorityOrder = { high: 0, medium: 1, low: 2 };
  ...
  visualizePlan(plan: ExecutionPlan): PlanVisualization {
    ...
    for (const stage of plan.stages) {
      const visualStage: VisualizationStage = {
        ...
        tasks: stage.tasks.map(task => ({
          ...
          dependencies: this.getTaskDependencies(task, plan),
        })),
    ...
    const criticalPathIds = plan.criticalPath.map(t => t.id);
   ...
   */
  private getTaskDependencies(task: Task, plan: ExecutionPlan): number[] {
    const deps: number[] = [];
