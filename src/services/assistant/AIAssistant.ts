...
export class AIAssistant {
  ...
  private async handleDeadlineQuery(
    ...
    return {
      type: 'list',
      content: content + deadlineTasks.map(t => `• **${t.title}** - ${t.dueDate} (${t.priority})`).join('\n'),
      data: deadlineTasks,
  ...
  private async handleNavigateQuery(
    ...
    const query = (parsed.entities.projectName || parsed.entities.taskName || parsed.originalQuery) as string;
  ...
  private async handleSearchQuery(
    ...
    const searchResults = await searchEngine.search(parsed.originalQuery, {}, { limit: 10, highlight: true });
  ...
  private async handleUnknownQuery(
    ...
    const searchResults = await searchEngine.search(parsed.originalQuery, {}, { limit: 5, fuzzy: true });
    ...
    return {
      type: 'text',
      content: '죄송합니다. 질문을 이해하지 못했습니다. 다른 방식으로 질문해 주시거나, "도움말"이라고 입력하여 사용 가능한 명령어를 확인하세요.',
  ...
  private async getAIResponse(query: string, context?: AssistantContext): Promise<string> {
    ...
    const systemPrompt = `당신은 AI Workflow Manager 앱의 AI 비서입니다. 사용자가 프로젝트와 태스크를 관리하는 것을 돕습니다.
