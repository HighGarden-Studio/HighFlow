...
export function useLocalAgentExecution() {
  ...
  const installedAgents = ref<Map<LocalAgentType, { installed: boolean; version?: string }>>(new Map());
   ...
   */
  async function checkInstalledAgents(): Promise<Map<LocalAgentType, { installed: boolean; version?: string }>> {
  ...
  function setupEventListeners() {
    ...
    const cleanupMessage = window.electron.localAgents.onSessionMessage((sessionId, message) => {
    ...
    const cleanupResponse = window.electron.localAgents.onSessionResponse((sessionId, response) => {
      if (currentSession.value?.id === sessionId) {
