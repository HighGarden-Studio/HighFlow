...
import { buildEnabledProvidersPayload, buildRuntimeMCPServers } from '../renderer/utils/runtimeConfig';
...
type ProgressPayload = { taskId: number; progress?: number; percentage?: number } & IPCExecutionProgress;
...
export function useTaskExecution() {
  ...
  async function executeTask(task: Task): Promise<ExecutionResult | null> {
    ...
    try {
      ...
      const config: AIConfig = {
        model: (task.aiProvider === 'anthropic' ? 'claude-3-5-sonnet-20250219' :
                task.aiProvider === 'openai' ? 'gpt-4-turbo' :
  ...
  const approvalRequest = ref<{ question: string; options?: string[]; context?: unknown } | null>(null);
  const cleanupFns = ref<(() => void)[]>([]);

  /**
   * Execute a task via IPC (for Electron main process execution)
   */
  async function executeTaskViaIPC(taskId: number, options?: { streaming?: boolean; timeout?: number }) {
    const api = getAPI();
    ...
    try {
      ...
      const fallbackProviders = enabledProviderPayload.length > 0
  ...
  async function pauseExecutionViaIPC(taskId?: number) {
    ...
    if (!api?.taskExecution || !id) return { success: false, error: 'API not available or no task ID' };
  ...
  function setupIPCListeners() {
    ...
    // Execution completed
    const cleanupCompleted = api.taskExecution.onCompleted((data: { taskId: number; result: unknown }) => {
      if (data.taskId === currentTaskId.value) {
        isExecuting.value = false;
        progress.value = 100;
        if (executionStats.value) {
          executionStats.value.endTime = Date.now();
          executionStats.value.duration = executionStats.value.endTime - executionStats.value.startTime;
