...
const agents = reactive<LocalAgent[]>([
  {
    id: 'claude-code',
    name: 'Claude Code',
    description: 'Anthropic의 공식 AI 코딩 에이전트. 터미널에서 자연어로 코딩 작업을 수행합니다.',
    icon: '🤖',
  ...
  <div class="local-agents-tab">
    ...
    <div class="mb-6">
      ...
      <p class="text-gray-400 text-sm">
        로컬 환경에서 실행되는 AI 코딩 에이전트를 관리합니다.
        이 도구들은 터미널에서 실행되며 별도의 API 연동이 필요하지 않습니다.
      </p>
    </div>
    ...
    <div class="space-y-4">
      ...
      >
        <div class="flex items-start justify-between">
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="text-gray-400 text-sm mt-1 max-w-md">{{ agent.description }}</p>

              <!-- API Key Requirement -->
              <div v-if="agent.requiresApiKey" class="flex items-center gap-2 mt-2 text-xs text-gray-500">
        ...
        <div v-if="!agent.isInstalled && !agent.isChecking" class="mt-4 pt-4 border-t border-gray-700">
          <div class="flex items-center justify-between">
            <div class="flex-1">
              <p class="text-xs text-gray-500 mb-2">설치 명령어:</p>
              <code class="block px-3 py-2 bg-gray-900 rounded-lg text-sm text-green-400 font-mono">
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          <!-- Modal -->
          <div class="relative w-full max-w-lg bg-gray-900 rounded-xl shadow-2xl border border-gray-700 overflow-hidden">
            <!-- Header -->
            ...
            <div class="p-6 space-y-4">
              ...
              <div class="p-4 bg-gray-800 rounded-lg space-y-3">
                ...
                <div v-if="selectedAgent.version" class="flex items-center justify-between">
                  <span class="text-gray-400 text-sm">버전</span>
                  <span class="text-white text-sm">{{ selectedAgent.version }}</span>
                </div>
                <div class="flex items-center justify-between">
                  <span class="text-gray-400 text-sm">실행 명령어</span>
                  <code class="px-2 py-1 bg-gray-900 rounded text-sm text-green-400 font-mono">
                    {{ selectedAgent.command }}
                  </code>
                </div>
                <div v-if="selectedAgent.requiresApiKey" class="flex items-center justify-between">
                  <span class="text-gray-400 text-sm">필요 환경변수</span>
                  <code class="px-2 py-1 bg-gray-900 rounded text-sm text-amber-400 font-mono">
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  <code class="flex-1 px-3 py-2 bg-gray-800 rounded-lg text-sm text-green-400 font-mono">
            ...
            <div class="px-6 py-4 border-t border-gray-700 bg-gray-800/50 flex justify-end gap-3">
              ...
              <button
                ...
                @click="launchAgent(selectedAgent); closeDetailModal()"
                :disabled="isLaunching"
                class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
              >
                터미널에서 실행
