...
const supportsOAuth = computed(() => {
  ...
  return props.provider.authMethods.includes('oauth') || props.provider.authMethods.includes('both');
});
...
const supportsApiKey = computed(() => {
  ...
  return props.provider.authMethods.includes('apiKey') || props.provider.authMethods.includes('both');
});
  ...
  try {
    ...
    if (isLocalProvider.value) {
      ...
      if (response?.ok) {
        ...
        validationMessage.value = 'Cannot connect to local server. Make sure it is running.';
      }
      return;
    }
        ...
        <div class="relative w-full max-w-lg transform transition-all">
          <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
            ...
            <div class="relative px-6 py-5 border-b border-gray-200 dark:border-gray-700">
              <div :class="['absolute inset-0 opacity-10 bg-gradient-to-r', providerInfo.color]" />
              <div class="relative flex items-center gap-4">
                <div :class="['w-12 h-12 rounded-xl bg-gradient-to-br flex items-center justify-center text-white font-bold text-xl', providerInfo.color]">
            ...
            <div class="px-6 py-4 space-y-6 max-h-[calc(100vh-280px)] overflow-y-auto">
              ...
              <div class="flex flex-wrap gap-2">
                <span v-if="provider.supportsStreaming" class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">
                  Streaming
                </span>
                <span v-if="provider.supportsVision" class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
                  Vision
                </span>
                <span v-if="provider.supportsTools" class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                  Tools/Functions
                </span>
                <span v-if="provider.maxTokens" class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
              ...
              <div v-if="supportsOAuth && (form.activeAuthMethod === 'oauth' || !supportsApiKey)" class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="text-sm font-medium text-blue-900 dark:text-blue-100">
                        {{ provider.isConnected ? 'Connected via OAuth' : 'Connect with OAuth' }}
                      </p>
                      <p class="text-xs text-blue-700 dark:text-blue-300">
                        {{ provider.isConnected ? 'Your account is linked' : 'Securely connect your account' }}
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div v-if="requiresBaseUrl || provider.baseUrl">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Base URL
                  <span v-if="!requiresBaseUrl" class="text-gray-400">(optional)</span>
                ...
                <input
                  ...
                  :placeholder="isLocalProvider ? 'http://localhost:11434' : 'https://api.example.com'"
                />
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <template v-else-if="isLocalProvider">
                    Local server URL (default: {{ provider.id === 'ollama' ? 'http://localhost:11434' : 'http://localhost:1234/v1' }})
                  </template>
                  <template v-else>
              ...
              <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    type="checkbox"
                    class="sr-only peer"
                    :disabled="!form.apiKey && !provider.isConnected && !isLocalProvider"
                  />
              ...
              <!-- Last Validated -->
              <div v-if="provider.lastValidated" class="text-xs text-gray-500 dark:text-gray-400 text-center">
