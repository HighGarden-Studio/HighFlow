...
import { useSettingsStore, type MCPServerConfig, type MCPServerTag } from '../../renderer/stores/settingsStore';
...
const filteredServers = computed(() => {
  ...
  if (selectedTags.value.length > 0) {
    servers = servers.filter(s =>
      s.tags && s.tags.some(tag => selectedTags.value.includes(tag))
    );
  }
...
const connectedCount = computed(() =>
  settingsStore.mcpServers.filter(s => s.isConnected).length
);

const enabledCount = computed(() =>
  settingsStore.mcpServers.filter(s => s.enabled).length
);
  ...
  <div class="mcp-servers-tab">
    ...
    <div class="mb-6">
      ...
      <p class="text-gray-400 text-sm">
        Model Context Protocol 서버를 연동하여 AI가 다양한 도구를 사용할 수 있도록 설정합니다.
        <a href="https://modelcontextprotocol.io" target="_blank" class="text-blue-400 hover:underline ml-1">
          MCP 문서 보기
        </a>
      </p>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
      <div class="bg-gray-800 rounded-lg p-4">
        <div class="text-2xl font-bold text-white">{{ settingsStore.mcpServers.length }}</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        ...
        <div class="flex items-start gap-3 mb-3">
          <div class="w-10 h-10 rounded-lg bg-gray-700 flex items-center justify-center text-xl flex-shrink-0 group-hover:scale-110 transition-transform">
