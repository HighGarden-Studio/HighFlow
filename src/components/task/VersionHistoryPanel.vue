    ...
    <div class="flex" :class="compact ? 'flex-col' : ''">
      ...
      <div class="w-full md:w-1/3 border-r border-gray-700 p-4">
        <div class="space-y-2 max-h-96 overflow-y-auto">
          <div
            ...
            :class="{
              'bg-blue-600/20 border border-blue-500': selectedVersionId === version.id,
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-2">
                <span class="font-medium text-gray-200">v{{ version.versionNumber }}</span>
                <span
              ...
              <span class="text-xs text-gray-500">{{ formatSize(version.size) }}</span>
            </div>
            <div class="mt-1 text-xs text-gray-500">
              {{ formatDate(version.createdAt) }}
            </div>
            <div v-if="version.metadata.reason" class="mt-1 text-xs text-gray-400 truncate">
              {{ version.metadata.reason }}
            </div>
            <div v-if="version.metadata.labels?.length" class="mt-1 flex flex-wrap gap-1">
              <span
      ...
      <div class="flex-1 p-4">
        ...
        <div v-if="showDiff && currentDiff">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center space-x-2">
              <span class="text-sm text-gray-400">
                v{{ currentDiff.fromVersionNumber }} → v{{ currentDiff.toVersionNumber }}
              </span>
              <span class="text-xs px-2 py-0.5 bg-gray-700 rounded text-gray-300">
                +{{ currentDiff.stats.additions }} -{{ currentDiff.stats.deletions }} ~{{ currentDiff.stats.changes }}
              </span>
            </div>
            <button
              @click="cancelCompare"
          ...
          <div class="bg-gray-900 rounded-lg p-4 max-h-80 overflow-auto font-mono text-sm">
            <div
        ...
        <div v-else-if="selectedVersion">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4 text-sm">
            <div>
              ...
              <div class="text-gray-300">{{ formatDate(selectedVersion.createdAt) }}</div>
            </div>
            <div>
            ...
            <div v-if="selectedVersion.metadata.cost">
              ...
              <div class="text-gray-300">${{ selectedVersion.metadata.cost.toFixed(4) }}</div>
            </div>
            <div v-if="selectedVersion.metadata.tokens">
              <span class="text-gray-500">토큰</span>
              <div class="text-gray-300">{{ selectedVersion.metadata.tokens.toLocaleString() }}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="mt-2 bg-gray-900 rounded-lg p-4 max-h-64 overflow-auto">
              <pre class="text-sm text-gray-300 whitespace-pre-wrap font-mono">{{ selectedVersion.content.slice(0, 1000) }}{{ selectedVersion.content.length > 1000 ? '...' : '' }}</pre>
            </div>
          </div>
        </div>
