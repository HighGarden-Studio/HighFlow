      ...
      >
        ...
        <div class="relative w-full max-w-2xl mx-4 bg-gray-800 rounded-xl shadow-2xl border border-gray-700 overflow-hidden">
          ...
          <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-700">
            ...
            <kbd class="hidden sm:inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-gray-400 bg-gray-700 rounded">
              ESC
            </kbd>
          </div>

          <!-- Filter Tabs -->
          <div class="flex items-center gap-1 px-4 py-2 border-b border-gray-700 overflow-x-auto">
            <button
          ...
          <div class="max-h-[60vh] overflow-y-auto">
            ...
            <div v-if="!searchQuery.trim() && recentSearches.length === 0" class="px-4 py-8 text-center">
              <div class="w-12 h-12 mx-auto mb-3 rounded-full bg-gray-700 flex items-center justify-center">
                <svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
              <p class="text-gray-400 text-sm">검색어를 입력하여 프로젝트, 태스크, 스킬 등을 찾아보세요</p>
              <div class="mt-4 flex flex-wrap justify-center gap-2 text-xs">
                <span class="text-gray-500">빠른 팁:</span>
                <kbd class="px-1.5 py-0.5 bg-gray-700 text-gray-400 rounded">↑↓</kbd>
                <span class="text-gray-500">탐색</span>
                <kbd class="px-1.5 py-0.5 bg-gray-700 text-gray-400 rounded">Enter</kbd>
            ...
            <!-- Recent Searches -->
            <div v-else-if="!searchQuery.trim() && recentSearches.length > 0" class="py-2">
              <div class="flex items-center justify-between px-4 py-2">
                <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">최근 검색</span>
                <button
              ...
              <div
                ...
                :class="index === selectedIndex ? 'bg-gray-700' : 'hover:bg-gray-700/50'"
              >
            ...
            <!-- No Results -->
            <div v-else-if="searchQuery.trim() && !loading && !hasResults" class="px-4 py-8 text-center">
              <div class="w-12 h-12 mx-auto mb-3 rounded-full bg-gray-700 flex items-center justify-center">
                <svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <p class="text-gray-400 text-sm">"{{ searchQuery }}"에 대한 결과가 없습니다</p>
            ...
            <div v-else-if="hasResults" class="py-2">
              ...
              <template v-if="(groupedResults.project?.length ?? 0) > 0">
                <div class="px-4 py-2">
                  <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">프로젝트</span>
                </div>
                <div
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="text-white font-medium truncate" v-html="result.highlightedTitle || result.title" />
                    <div class="text-gray-400 text-sm truncate" v-html="result.highlightedSnippet || result.snippet" />
                  </div>
                </div>
              </template>
              ...
              <template v-if="(groupedResults.task?.length ?? 0) > 0">
                <div class="px-4 py-2 mt-2">
                  <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">태스크</span>
                </div>
                <div
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                      <span class="text-white font-medium truncate" v-html="result.highlightedTitle || result.title" />
                      <span
                        v-if="result.metadata.status"
                        class="px-1.5 py-0.5 text-xs rounded"
                        :class="{
                          'bg-gray-600 text-gray-300': result.metadata.status === 'todo',
                          'bg-blue-500/20 text-blue-400': result.metadata.status === 'in_progress',
                          'bg-orange-500/20 text-orange-400': result.metadata.status === 'needs_approval',
                          'bg-yellow-500/20 text-yellow-400': result.metadata.status === 'in_review',
                          'bg-green-500/20 text-green-400': result.metadata.status === 'done',
                        }"
                      >
              ...
              <template v-if="(groupedResults.skill?.length ?? 0) > 0">
                <div class="px-4 py-2 mt-2">
                  <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">스킬</span>
                </div>
                <div
              ...
              <template v-if="(groupedResults.user?.length ?? 0) > 0">
                <div class="px-4 py-2 mt-2">
                  <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">사용자</span>
                </div>
                <div
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="text-white font-medium truncate" v-html="result.highlightedTitle || result.title" />
                    <div class="text-gray-400 text-sm truncate">{{ result.metadata.email }}</div>
                  </div>
                </div>
              </template>
            </div>
          ...
          <div v-if="suggestions.length > 0 && searchQuery" class="px-4 py-2 border-t border-gray-700">
