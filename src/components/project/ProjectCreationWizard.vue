...
import { promptEnhancementService, type EnhancementOptions } from '../../services/prompt/PromptEnhancementService';
...
const CHAT_CAPABLE_PROVIDERS = [
  ...
  'mistral', 'groq', 'deepseek', 'cohere',
  'openrouter', 'together', 'fireworks', 'perplexity',
  'ollama', 'lmstudio',
  'zhipu', 'moonshot', 'qwen', 'baidu',
];
...
const STEPS: { id: WizardStep; title: string; description: string }[] = [
  ...
  { id: 'concretize', title: '아이디어 구체화', description: '수집된 정보를 기반으로 아이디어를 구체화합니다' },
  { id: 'preview', title: '실행 계획', description: '생성된 태스크와 AI 모델 추천을 확인하세요' },
  { id: 'optimize', title: '프롬프트 최적화', description: '각 태스크의 프롬프트를 최적화합니다' },
...
watch(availableChatProviders, (providers) => {
...
const AI_DELEGATE_PRESETS = [
  { id: 'ai-decide', label: 'AI 판단에 맡기기', description: 'AI가 최선의 선택을 하도록 위임합니다' },
  { id: 'simple', label: '가장 간단한 방법으로', description: '복잡도를 최소화하는 방향으로 진행합니다' },
  { id: 'best-practice', label: '업계 표준/베스트 프랙티스로', description: '일반적으로 권장되는 방식을 따릅니다' },
  { id: 'skip', label: '이 부분은 나중에 결정', description: '해당 질문은 건너뛰고 나중에 결정합니다' },
];
...
const canProceed = computed(() => {
  switch (currentStep.value) {
    ...
    case 'interview':
      return interviewSession.value?.status === 'completed' ||
...
const isDevProject = computed(() => {
  const stack = executionPlan.value?.architecture
    ? executionPlan.value.architecture
    : '';
  ...
  const hasCodeOutput = wizardTasks.value.some((task) => {
    const format = (task.primaryOutputFormat || task.expectedOutputFormat || '').toString().toLowerCase();
...
const wizardTasks = computed<OptimizedTask[]>(() => {
  if (executionPlan.value) {
    return executionPlan.value.tasks.map((task, index) => {
      const provider = (task.recommendedProviders?.[0] as AIProvider | undefined) || 'anthropic';
      const expectedFormat = (task.expectedOutputFormat as TaskOutputFormat | undefined) || 'markdown';
      return {
        title: task.title,
...
function getRepoTypeColor(type: DiscoveredRepo['type']): string {
  switch (type) {
    case 'git': return 'bg-orange-500';
    case 'claude-code': return 'bg-violet-500';
    case 'codex': return 'bg-cyan-500';
    case 'antigravity': return 'bg-rose-500';
...
function startInterview() {
  ...
  const checkFallback = setInterval(() => {
    ...
    if (updatedSession) {
      ...
      if (updatedSession.fallbackOccurred && updatedSession.aiProvider !== selectedProvider.value) {
...
function selectPresetAnswer(preset: typeof AI_DELEGATE_PRESETS[number]) {
  const messages: Record<string, string> = {
    'ai-decide': 'AI가 최선이라고 판단하는 방식으로 진행해주세요. 전문적인 판단에 맡기겠습니다.',
    ...
    if (executionPlan.value) {
      ...
      executionPlan.value = {
        ...
        tasks: executionPlan.value.tasks.map((task, index) => ({
          ...task,
          recommendedProviders:
            task.recommendedProviders?.length
...
async function addRequirementAndGenerateTasks() {
  if (!newRequirementInput.value.trim() || !concretizedIdea.value || !interviewSession.value) return;
...
function buildFallbackRecommendation(task: OptimizedTask): ModelRecommendation | null {
  const providerId = (task.recommendedProviders?.[0] as AIProvider | undefined) || task.suggestedAIProvider;
  if (!providerId) {
    return null;
  }
...
async function optimizeTasks() {
  ...
  try {
    const tasksToOptimize = wizardTasks.value
      .filter((_, idx) => selectedTasks.value.has(idx.toString()));
    ...
    for (const [i, task] of tasksToOptimize.entries()) {
      ...
      if (!task.aiOptimizedPrompt && analysis.overallScore < 70) {
        const enhancementFormat = mapOutputFormatToEnhancementFormat(task.primaryOutputFormat);
...
async function createProject() {
  ...
  const project = {
    ...
    tasks: optimizedTasks.value.map(task => ({
      ...
      expectedOutputFormat: task.expectedOutputFormat || task.primaryOutputFormat || 'markdown',
      outputFormat: task.primaryOutputFormat || 'markdown',
      recommendedProviders: task.recommendedProviders || (task.suggestedAIProvider ? [task.suggestedAIProvider] : []),
      requiredMCPs: task.requiredMCPs || [],
      codeLanguage: task.codeLanguage,
      tags: (task.tags && task.tags.length > 0) ? task.tags : task.category ? [task.category] : [],
...
<template>
  <Teleport to="body">
    ...
    >
      <div class="w-full max-w-5xl max-h-[90vh] bg-gray-900 rounded-xl shadow-2xl flex flex-col overflow-hidden border border-gray-700">
        <!-- Header -->
            ...
            <template v-for="(step, idx) in STEPS" :key="step.id">
              ...
              >
                <span class="w-5 h-5 flex items-center justify-center rounded-full text-xs"
                  :class="idx < currentStepIndex ? 'bg-green-500 text-white' : 'bg-gray-600'"
                >
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div v-if="currentStep === 'idea'" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <button
                @click="creationMode = 'local-repo'; scanLocalRepositories()"
                :class="[
                  'p-4 border-2 rounded-xl text-left transition-all',
            ...
            <div v-if="creationMode === 'ai-wizard'" class="space-y-6">
              ...
              <div class="bg-blue-900/20 border border-blue-800/30 rounded-lg p-4">
                <div class="flex items-start gap-3">
                  <span class="text-2xl">💡</span>
                  <div>
                    <h4 class="font-medium text-blue-300">팁: 좋은 아이디어 설명이란?</h4>
                    <ul class="text-sm text-gray-400 mt-2 space-y-1">
                      <li>• 해결하고자 하는 문제나 목표를 명확히 설명하세요</li>
            ...
            <div v-if="creationMode === 'local-repo'" class="space-y-4">
              ...
              <div class="flex items-center gap-3">
                ...
                >
                  ...
                  <span>{{ isScanning ? '검색 중...' : '저장소 다시 검색' }}</span>
                </button>
              ...
              <div v-if="isScanning" class="text-center py-12">
                <div class="animate-spin w-10 h-10 border-4 border-violet-500 border-t-transparent rounded-full mx-auto mb-3"></div>
                <p class="text-gray-400">로컬 저장소를 검색하고 있습니다...</p>
                <p class="text-xs text-gray-500 mt-1">Development, Projects, GitHub 폴더 등을 확인합니다</p>
              </div>

              <div v-else-if="filteredRepos.length === 0" class="text-center py-12 bg-gray-800/50 rounded-lg">
                <span class="text-4xl mb-3 block">📭</span>
                <p class="text-gray-400">발견된 저장소가 없습니다</p>
                <p class="text-xs text-gray-500 mt-1">폴더 직접 선택 버튼을 사용하여 저장소를 추가하세요</p>
              </div>
                ...
                >
                  <div class="flex items-start gap-3">
                    ...
                    <div :class="[
                      ...
                      <svg v-if="selectedRepo?.path === repo.path" class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 mb-1">
                        <span :class="['w-6 h-6 rounded flex items-center justify-center text-white text-xs', getRepoTypeColor(repo.type)]">
                          {{ getRepoTypeIcon(repo.type) }}
                        </span>
                        <h4 class="font-medium text-white truncate">{{ repo.name }}</h4>
                        <span class="px-2 py-0.5 text-xs rounded bg-gray-700 text-gray-300">{{ repo.type }}</span>
                      </div>
                      <p class="text-xs text-gray-500 truncate mb-1">{{ repo.path }}</p>
                      <div class="flex items-center gap-3 text-xs text-gray-500">
                        <span>{{ formatRelativeTime(repo.lastModified) }}</span>
                        <span v-if="repo.remoteUrl" class="flex items-center gap-1">
                          <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M12.586 4.586a2 2 0 112.828 2.828l-3 3a2 2 0 01-2.828 0 1 1 0 00-1.414 1.414 4 4 0 005.656 0l3-3a4 4 0 00-5.656-5.656l-1.5 1.5a1 1 0 101.414 1.414l1.5-1.5zm-5 5a2 2 0 012.828 0 1 1 0 101.414-1.414 4 4 0 00-5.656 0l-3 3a4 4 0 105.656 5.656l1.5-1.5a1 1 0 10-1.414-1.414l-1.5 1.5a2 2 0 11-2.828-2.828l3-3z" clip-rule="evenodd" />
                          </svg>
                          <span class="truncate max-w-[200px]">{{ repo.remoteUrl }}</span>
                        </span>
                      </div>
                      <p v-if="repo.description" class="text-sm text-gray-400 mt-2 line-clamp-2">{{ repo.description }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <!-- Selected repo info -->
              <div v-if="selectedRepo" class="bg-violet-900/20 border border-violet-800/30 rounded-lg p-4">
                <div class="flex items-start gap-3">
                  <span class="text-xl">✅</span>
                  <div class="flex-1">
                    <h4 class="font-medium text-violet-300">선택된 저장소</h4>
                    <p class="text-sm text-gray-400 mt-1">{{ selectedRepo.name }} ({{ selectedRepo.type }})</p>
                    <p class="text-xs text-gray-500 mt-1">{{ selectedRepo.path }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="currentStep === 'provider'" class="space-y-6">
            ...
            <div v-if="availableChatProviders.length === 0" class="bg-yellow-900/20 border border-yellow-800/30 rounded-lg p-4">
              <div class="flex items-start gap-3">
                <span class="text-2xl">⚠️</span>
                <div>
                  <h4 class="font-medium text-yellow-300">활성화된 AI 프로바이더가 없습니다</h4>
                  <p class="text-sm text-gray-400 mt-1">
                    AI 인터뷰를 진행하려면 Settings > AI Providers에서 최소 하나의 AI 프로바이더를 설정하고 활성화해야 합니다.
                  </p>
                  <button
                    @click="emit('close')"
            ...
            <div v-else class="grid grid-cols-2 gap-4">
              ...
              >
                <div class="flex items-center gap-3 mb-3">
                  <div :class="['w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold', getProviderColorById(provider.id)]">
                    {{ provider.name.charAt(0) }}
                  </div>
                  <div>
                    <div class="font-semibold text-white">{{ provider.name }}</div>
                    <div class="text-xs text-gray-500">{{ provider.defaultModel }}</div>
                  </div>
                </div>
                ...
                <div class="flex flex-wrap gap-1 mt-2">
                  <span v-if="provider.supportsStreaming" class="text-xs px-2 py-0.5 bg-green-900/30 text-green-400 rounded">Streaming</span>
                  <span v-if="provider.supportsVision" class="text-xs px-2 py-0.5 bg-purple-900/30 text-purple-400 rounded">Vision</span>
                  <span v-if="provider.supportsTools" class="text-xs px-2 py-0.5 bg-blue-900/30 text-blue-400 rounded">Tools</span>
                </div>
              </button>
            </div>
          </div>
          ...
          <div v-if="currentStep === 'interview'" class="flex flex-col h-full">
            ...
            <div class="mb-4 bg-gray-800 rounded-lg p-3">
              <div class="flex items-center justify-between text-sm mb-2">
                ...
                <span :class="interviewProgress >= 70 ? 'text-green-400' : 'text-yellow-400'">
                  {{ interviewProgress }}%
                </span>
              </div>
              <div class="h-2 bg-gray-700 rounded-full overflow-hidden">
                <div
                  ...
                  :class="interviewProgress >= 70 ? 'bg-green-500' : 'bg-yellow-500'"
                  :style="{ width: `${interviewProgress}%` }"
                ></div>
              </div>
              <p class="text-xs text-gray-500 mt-2">
                {{ interviewProgress >= 70 ? '충분한 정보가 수집되었습니다. 다음 단계로 진행할 수 있습니다.' : 'AI의 질문에 답변하여 아이디어를 구체화하세요.' }}
              </p>
            </div>

            <!-- Chat Messages -->
            <div ref="chatContainer" class="flex-1 bg-gray-800/50 rounded-lg p-4 overflow-y-auto min-h-[300px] max-h-[400px] space-y-3">
              <div
              ...
              >
                ...
                <div v-if="message.role === 'user'" class="text-sm text-white whitespace-pre-wrap">{{ message.content }}</div>
                <!-- AI/System messages: streaming 표시 -->
                <div v-else>
                  <div v-if="message.metadata?.typing" class="flex items-center gap-3">
                    <div class="relative w-8 h-8">
                      ...
                      <div class="absolute inset-0">
                        <svg class="w-8 h-8 animate-spin-slow" viewBox="0 0 32 32">
                          <circle cx="16" cy="16" r="14" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-600" />
                          <path d="M16 2 A14 14 0 0 1 30 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" class="text-blue-400" />
                        </svg>
                      </div>
                    </div>
                  ...
                  <div v-else>
                    <div class="text-sm text-white markdown-content" v-html="renderMarkdown(message.content)"></div>
                    <div
                    ...
                    <div class="text-xs text-gray-400 mt-1 flex items-center gap-2">
                      ...
                      >
                        <span class="inline-block w-2 h-2 bg-blue-400 rounded-full animate-bounce" style="animation-delay: 0ms;"></span>
                        <span class="inline-block w-2 h-2 bg-blue-400 rounded-full animate-bounce" style="animation-delay: 150ms;"></span>
                        <span class="inline-block w-2 h-2 bg-blue-400 rounded-full animate-bounce" style="animation-delay: 300ms;"></span>
                      </span>
                    </div>
                  </div>
                </div>
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                  <div class="text-sm text-white font-medium mb-1">AI가 답변을 준비 중입니다...</div>
            ...
            <div class="mt-4 bg-gray-800/30 rounded-lg p-3">
              <p class="text-xs text-gray-500 mb-2">💡 답변이 어려우시면 아래 옵션을 선택하세요:</p>
            ...
            <!-- Force Complete -->
            <div v-if="interviewProgress >= 50 && interviewProgress < 70" class="mt-3 text-center">
              <button
          ...
          <div v-if="currentStep === 'concretize'" class="space-y-6">
            ...
            <div v-else-if="concretizedIdea" class="space-y-6">
              <div class="bg-gray-800 rounded-lg p-4">
                <h3 class="font-semibold text-white mb-2">{{ concretizedIdea.title }}</h3>
                <p class="text-gray-400 text-sm whitespace-pre-wrap">{{ concretizedIdea.summary }}</p>
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div class="bg-gray-800 rounded-lg p-4">
                  <h4 class="text-sm font-medium text-gray-400 mb-2">기술 스택</h4>
                  <div class="flex flex-wrap gap-2">
                    <span
                      v-for="tech in concretizedIdea.technicalSpecification.stack"
                      :key="tech"
                ...
                <div class="bg-gray-800 rounded-lg p-4">
                  ...
                  <div class="flex items-center gap-2">
                    <span :class="[
                      ...
                      concretizedIdea.estimatedComplexity === 'simple' ? 'bg-green-900/30 text-green-400' :
                      concretizedIdea.estimatedComplexity === 'moderate' ? 'bg-yellow-900/30 text-yellow-400' :
                      concretizedIdea.estimatedComplexity === 'complex' ? 'bg-orange-900/30 text-orange-400' :
              ...
              <div class="bg-gray-800 rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                  <h4 class="text-sm font-medium text-gray-400">생성된 태스크 미리보기</h4>
                  <button
                ...
                <!-- 요구사항 추가 입력 영역 -->
                <div v-if="showAddRequirement" class="mb-4 p-3 bg-blue-900/20 border border-blue-800/30 rounded-lg">
                  <p class="text-xs text-blue-300 mb-2">추가하고 싶은 요구사항이나 기능을 설명해주세요:</p>
                  <textarea
                  ...
                  <div class="flex justify-end gap-2 mt-2">
                    <button
                      @click="showAddRequirement = false; newRequirementInput = ''"
                    ...
                    <button
                      ...
                      :disabled="!newRequirementInput.trim() || isAddingRequirement"
                    ...
                    >
                      <svg v-if="isAddingRequirement" class="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                      </svg>
                      {{ isAddingRequirement ? '태스크 생성 중...' : '태스크 추가 생성' }}
                    </button>
                  </div>
                </div>
                  ...
                  >
                    <div class="flex-1">
                      ...
                      <div class="text-xs text-gray-500">
                        {{ task.category }} • {{ task.estimatedMinutes }}분 예상
                      </div>
                    </div>
                    <div :class="['w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs', getProviderColor(task.suggestedAIProvider)]">
              ...
              >
                <div class="flex items-start gap-3">
                  <div :class="[
                    ...
                    <svg v-if="selectedTasks.has(idx.toString())" class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                    </svg>
                  </div>
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between mb-1">
                      ...
                      <span :class="['text-xs', getComplexityColor(task.complexity)]">
                        {{ task.complexity }}
                      </span>
                    </div>
                    <p class="text-sm text-gray-400 mb-2 line-clamp-2">{{ task.description }}</p>

                    <div class="flex flex-wrap items-center gap-2 mb-2 text-xs">
                      <span class="px-2 py-0.5 rounded bg-purple-500/20 text-purple-200 border border-purple-500/40">
                        {{ task.primaryOutputFormat || task.expectedOutputFormat || 'markdown' }}
                      </span>
                      <span class="px-2 py-0.5 rounded bg-amber-500/20 text-amber-200 border border-amber-500/40" v-if="task.estimatedMinutes">
                        {{ task.estimatedMinutes }}분 예상
                      </span>
                    </div>

                    <!-- AI Model Recommendation -->
                    <div v-if="taskRecommendations.get(task.title)" class="bg-gray-700/50 rounded-lg p-3">
                      <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                          <div :class="['w-6 h-6 rounded flex items-center justify-center text-white text-xs', getProviderColor(taskRecommendations.get(task.title)!.model.provider)]">
                            {{ getProviderIcon(taskRecommendations.get(task.title)!.model.provider) }}
                          </div>
                          <span class="text-sm font-medium text-white">
                            {{ taskRecommendations.get(task.title)!.model.displayName }}
                          </span>
                        </div>
                        <div class="text-xs text-gray-500">
                          {{ formatCost(taskRecommendations.get(task.title)!.estimatedCost) }} 예상
                        </div>
                      </div>
                      <div class="flex flex-wrap gap-1">
                        <span
                          v-for="reason in taskRecommendations.get(task.title)!.reasons.slice(0, 2)"
                          :key="reason"
            ...
            <div class="border-t border-gray-700 pt-4">
              <div class="bg-gray-800/50 rounded-lg p-3">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <span class="text-sm text-gray-400">추가적으로 원하는 기능이 있으신가요?</span>
                </div>
                <div class="flex gap-2">
                  ...
                  >
                    <svg v-if="isAddingRequirement" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    ...
                    <span>{{ isAddingRequirement ? '생성 중...' : '태스크 추가' }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div v-if="currentStep === 'optimize'" class="space-y-6">
            <div v-if="isOptimizing" class="text-center py-8">
              ...
              <div class="w-64 h-2 bg-gray-700 rounded-full mx-auto mt-3 overflow-hidden">
                <div
                  class="h-full bg-purple-500 transition-all"
                  :style="{ width: `${((currentOptimizingTask + 1) / selectedTasks.size) * 100}%` }"
                ></div>
              </div>
            </div>
            ...
            <div v-else class="space-y-4">
              <div class="bg-green-900/20 border border-green-800/30 rounded-lg p-4">
                <div class="flex items-center gap-2 text-green-400">
                  ...
                  <span class="font-medium">{{ optimizedTasks.length }}개 태스크 프롬프트 최적화 완료!</span>
                </div>
              </div>
              ...
              <div class="space-y-3 max-h-[350px] overflow-y-auto">
                ...
                >
                  <div class="flex items-center justify-between mb-2">
                    <h4 class="font-medium text-white">{{ task.title }}</h4>
                    <div class="flex items-center gap-2">
                      <div :class="['w-6 h-6 rounded flex items-center justify-center text-white text-xs', getProviderColor(task.suggestedAIProvider)]">
                        {{ getProviderIcon(task.suggestedAIProvider) }}
                      </div>
                      <span class="text-xs text-gray-500">{{ task.suggestedModel }}</span>
                    </div>
                  </div>
                  <div class="flex flex-wrap items-center gap-2 mb-2 text-xs">
                    <span class="px-2 py-0.5 rounded bg-purple-500/20 text-purple-200 border border-purple-500/40">
                    ...
                    <span class="px-2 py-0.5 rounded bg-gray-500/20 text-gray-200 border border-gray-500/40" v-if="task.executionOrder">
                      #{{ task.executionOrder }}
                    </span>
                  </div>
                  <div class="bg-gray-900 rounded p-3 max-h-32 overflow-y-auto">
                    <pre class="text-xs text-gray-400 whitespace-pre-wrap">{{ task.description.substring(0, 300) }}{{ task.description.length > 300 ? '...' : '' }}</pre>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="currentStep === 'confirm'" class="space-y-6">
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">프로젝트 제목</label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">프로젝트 설명</label>
                <textarea
              ...
              <div v-if="isDevProject" class="space-y-2">
                <div class="flex items-center justify-between">
                  <label class="block text-sm font-medium text-gray-300">개발 베이스 폴더</label>
                  <span class="text-xs text-gray-400">로컬 개발을 시작할 기본 경로</span>
                </div>
                <div class="flex gap-2">
                  <input
            ...
            <div class="bg-gray-800 rounded-lg p-4 space-y-3">
              ...
              <div class="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <div class="text-white font-medium">{{ optimizedTasks.length }}개</div>
                </div>
                <div>
                  <div class="text-gray-500">예상 시간</div>
                  <div class="text-white font-medium">
                    {{ Math.round(optimizedTasks.reduce((sum, t) => sum + t.estimatedMinutes, 0) / 60) }}시간
                  </div>
                </div>
                <div>
                  <div class="text-gray-500">AI 모델</div>
                  <div class="text-white font-medium">
                    {{ new Set(optimizedTasks.map(t => t.suggestedAIProvider)).size }}개 사용
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-blue-900/20 border border-blue-800/30 rounded-lg p-4">
              <div class="flex items-start gap-3">
                <span class="text-xl">🚀</span>
                <div>
                  <h4 class="font-medium text-blue-300">프로젝트 생성 준비 완료!</h4>
                  <p class="text-sm text-gray-400 mt-1">
                    "프로젝트 생성" 버튼을 클릭하면 설정된 태스크들이 프로젝트에 등록됩니다.
                    각 태스크는 추천된 AI 모델과 최적화된 프롬프트로 실행될 준비가 됩니다.
                  </p>
                </div>
              </div>
            </div>
        ...
        <!-- Footer -->
        <div class="px-6 py-4 border-t border-gray-700 bg-gray-800/50 flex items-center justify-between">
          <button
          ...
          <div class="flex items-center gap-3">
            ...
            <button
              v-if="currentStep === 'idea' && creationMode === 'local-repo' && selectedRepo"
            ...
            >
              {{ creationMode === 'local-repo' && currentStep === 'idea' ? 'AI로 태스크 생성 →' : '다음 →' }}
            </button>
