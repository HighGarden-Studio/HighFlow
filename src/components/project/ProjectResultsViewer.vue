...
// ProjectOutput is defined for future use with file system integration
type _ProjectOutput = {
  type: 'web' | 'document' | 'image' | 'video' | 'code' | 'data' | 'other';
  path?: string;
  files?: OutputFile[];
  isReady: boolean;
  lastUpdated?: Date;
...
watch(() => props.project.outputPath, () => {
  loadOutputFiles();
});
</script>
  ...
  <div class="project-results-viewer bg-gray-800/50 rounded-lg border border-gray-700">
    ...
    <div class="flex border-b border-gray-700">
      <button
        ...
        :class="activeTab === 'results' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-200'"
      >
        실행 결과
      </button>
      <button
        ...
        class="px-4 py-2 text-sm font-medium transition-colors"
        :class="activeTab === 'preview' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-200'"
      >
        미리보기
      </button>
      <button
        ...
        class="px-4 py-2 text-sm font-medium transition-colors"
        :class="activeTab === 'files' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-200'"
      >
        파일
      </button>
    </div>
    ...
    <div class="p-4">
      ...
      <div v-if="activeTab === 'results'" class="space-y-4">
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <div class="bg-gray-900/30 rounded-lg p-3 text-center">
            <div class="text-2xl font-bold text-blue-400">${{ aggregatedCost.toFixed(4) }}</div>
            <div class="text-xs text-gray-500">총 비용</div>
          </div>
          <div class="bg-gray-900/30 rounded-lg p-3 text-center">
            <div class="text-2xl font-bold text-purple-400">{{ outputFiles.length }}</div>
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-2">
                ...
                <span class="text-sm text-gray-200">{{ execution.taskTitle || `태스크 #${execution.taskId}` }}</span>
              </div>
              <span class="text-xs text-gray-500">{{ execution.model }}</span>
            </div>
            <div v-if="execution.response" class="mt-2 text-xs text-gray-400 line-clamp-2">
      ...
      <div v-else-if="activeTab === 'preview'" class="space-y-4">
        ...
        <div v-if="canPreview && !previewUnavailableReason">
          ...
          <div v-if="outputType === 'web'" class="space-y-3">
            ...
            <div class="aspect-video bg-gray-900 rounded-lg overflow-hidden border border-gray-600">
              <iframe
          ...
          <div v-else-if="outputType === 'image'" class="space-y-3">
            ...
            <div class="grid grid-cols-3 gap-2">
              <div
                v-for="file in outputFiles.filter(f => ['png', 'jpg', 'jpeg', 'gif', 'svg'].includes(f.type))"
                :key="file.path"
          ...
          <div v-else-if="outputType === 'video'" class="space-y-3">
            ...
            <div
              v-for="file in outputFiles.filter(f => ['mp4', 'webm', 'mov'].includes(f.type))"
              :key="file.path"
          ...
          <div v-else-if="outputType === 'document' || outputType === 'code'" class="space-y-3">
