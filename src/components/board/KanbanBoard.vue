  ...
  return props.projects.map(project => {
    ...
    if (props.filterBy?.assignee && props.filterBy.assignee.length > 0) {
      tasks = tasks.filter((task) =>
        task.assigneeId && props.filterBy?.assignee?.includes(task.assigneeId)
      );
    }
...
function handleTaskRetry(task: Task) {
  ...
  const updatedTask = { ...task, status: 'in_progress' as any };
  emit('taskUpdate', updatedTask);
  emit('taskRetry', task);
}
...
<template>
  <div class="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      ...
      <div class="flex items-center justify-between">
        ...
        <div>
          ...
          <div class="flex items-center gap-4 mt-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <span class="text-blue-600 dark:text-blue-400">{{ statistics.inProgress }} in progress</span>
            <span class="text-green-600 dark:text-green-400">{{ statistics.completed }} completed</span>
            <span v-if="statistics.blocked > 0" class="text-red-600 dark:text-red-400">
              {{ statistics.blocked }} blocked
            </span>
            <span class="ml-2 px-2 py-0.5 rounded bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 font-semibold">
        ...
        <div class="flex items-center gap-4">
          ...
          <!-- Collaboration Users -->
          <div v-if="showCollaboration && otherUsers.length > 0" class="flex items-center gap-2">
    ...
    <div class="flex-1 overflow-y-auto" style="overscroll-behavior: contain;">
      ...
      <div class="sticky top-0 z-10 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <!-- Task Count Badge -->
              <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                {{ totalTasks }} tasks
              </span>
            </div>
            ...
            <div class="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
              <span v-if="tasksByColumn.done" class="text-green-600 dark:text-green-400">
                {{ tasksByColumn.done.length }} completed
              </span>
              <span v-if="tasksByColumn.in_progress" class="text-blue-600 dark:text-blue-400">
                {{ tasksByColumn.in_progress.length }} in progress
              </span>
              <span v-if="tasksByColumn.blocked && tasksByColumn.blocked.length > 0" class="text-red-600 dark:text-red-400">
