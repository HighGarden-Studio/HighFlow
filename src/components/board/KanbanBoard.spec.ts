    ...
    global: {
      stubs: {
        KanbanColumn: {
          ...
          emits: [
            'taskClick', 'taskEdit', 'addTask', 'dragstartTask',
            'taskExecute', 'taskEnhancePrompt', 'taskPreviewPrompt',
            'taskPreviewResult', 'taskRetry', 'taskViewHistory',
            'taskViewProgress', 'taskViewStepHistory', 'taskPause',
            'taskResume', 'taskStop', 'taskSubdivide',
          ],
        },
      },
    },
...
describe('KanbanBoard', () => {
  ...
  describe('Project Expansion', () => {
    it('should start with all projects expanded', async () => {
      ...
      expect(expandedColumns.length).toBe(10); // All columns visible
    });
    ...
    it('should collapse project when header is clicked', async () => {
      ...
      // First project should be collapsed
      const columns = wrapper.findAll('.kanban-column');
      expect(columns.length).toBe(5); // Only second project columns
    });
      ...
      const tasks = [
        {
          ...
          color: '#000',
          tasks: [
            createMockTask({ id: 1, title: 'Match All', priority: 'high', tags: ['frontend'], assigneeId: 1 }),
            createMockTask({ id: 2, title: 'Match Priority', priority: 'high', tags: ['backend'], assigneeId: 2 }),
            createMockTask({ id: 3, title: 'Match Tag', priority: 'low', tags: ['frontend'], assigneeId: 2 }),
          ],
        },
      ];
