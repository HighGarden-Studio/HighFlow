    ...
    for (const task of validTasks) {
        ...
        for (const depId of dependencies) {
            const edgeType: 'dependency' | 'blocking' | 'parent-child' =
                task.blockedByTaskId === depId
                    ? 'blocking'
                    : task.parentTaskId === depId
                      ? 'parent-child'
                      : 'dependency';
...
<template>
    <div class="dependency-graph-container">
            ...
            >
                ...
                <g class="edges">
                    <path
                        ...
                        fill="none"
                        :marker-end="
                            edge.isOnCriticalPath ? 'url(#arrowhead-critical)' : 'url(#arrowhead)'
                        "
                        class="edge"
                        :class="{
                ...
                <g class="nodes">
                    ...
                    >
                        ...
                        <rect
                            ...
                            :stroke="
                                selectedTaskId === node.id
                                    ? '#3B82F6'
                                    : getStatusColor(node.task.status).border
                            "
                        ...
                        <!-- Task status -->
                        <text :x="12" :y="48" fill="#9CA3AF" font-size="11">
                            {{ node.task.status.replace('_', ' ').toUpperCase() }}
                        </text>

                        <!-- Task ID -->
                        <text :x="12" :y="68" fill="#6B7280" font-size="10">
                            #{{ node.task.id }}
                        </text>

                        <!-- Subtask indicator -->
                        <g
                            v-if="node.task.subtaskCount && node.task.subtaskCount > 0"
                            :transform="`translate(${node.width - 50}, 55)`"
                        >
        ...
        <div v-if="hoveredNodeId" class="info-panel">
            <template
                v-for="node in graphNodes.filter((n) => n.id === hoveredNodeId)"
                :key="node.id"
            >
