...
test.describe('Project Creation Flow', () => {
    ...
    test('should navigate to project creation', async ({ page }) => {
        ...
        const createButton = page.locator(
            '[data-testid="create-project"], button:has-text("새 프로젝트"), button:has-text("Create Project")'
        );
    ...
    test('should display template selection options', async ({ page }) => {
        const createButton = page.locator(
            '[data-testid="create-project"], button:has-text("새 프로젝트")'
        );
        ...
        if (await createButton.isVisible()) {
            ...
            await page
                .waitForSelector('[data-template], [class*="template"]', { timeout: 5000 })
                .catch(() => {});
    ...
    test('should select a web app template', async ({ page }) => {
        ...
        if (await createButton.isVisible()) {
            ...
            const webAppTemplate = page.locator(
                '[data-template="web-app"], [data-template*="web"]'
            );
    ...
    test('should accept main prompt input', async ({ page }) => {
        ...
        const promptInput = page.locator(
            '[data-testid="main-prompt"], textarea[placeholder*="prompt"], textarea[placeholder*="프로젝트"]'
        );
    ...
    test('should validate empty prompt', async ({ page }) => {
        ...
        const submitButton = page.locator(
            '[data-testid="submit-prompt"], button[type="submit"], button:has-text("다음"), button:has-text("Next")'
        );
    ...
    test('should show AI provider selection', async ({ page }) => {
        ...
        const providerSelect = page.locator(
            '[data-testid="ai-provider"], select[name="provider"], [class*="provider-select"]'
        );
    ...
    test('should accept answers to follow-up questions', async ({ page }) => {
        ...
        const answerInput = page.locator(
            '[data-testid="answer-1"], input[name*="answer"], textarea[name*="answer"]'
        );
    ...
    test('should show task preview before creation', async ({ page }) => {
        ...
        if (await taskPreview.isVisible()) {
            ...
            const taskItems = page.locator(
                '[data-testid="preview-task"], [class*="preview-task-item"]'
            );
    ...
    test('should create project and redirect to board', async ({ page }) => {
        ...
        const createButton = page.locator(
            '[data-testid="create"], button:has-text("생성"), button:has-text("Create")'
        );
...
test.describe('Task Execution with AI', () => {
    ...
    test('should click on a task to open details', async ({ page }) => {
        ...
        if (await taskCard.isVisible()) {
            ...
            const detailPanel = page.locator(
                '[data-testid="task-detail-panel"], [class*="task-detail"]'
            );
    ...
    test('should show task detail panel with execute button', async ({ page }) => {
        ...
        if (await taskCard.isVisible()) {
            ...
            const executeButton = page.locator(
                '[data-testid="execute-task"], button:has-text("실행"), button:has-text("Execute")'
            );
    ...
    test('should start task execution', async ({ page }) => {
        ...
        if (await taskCard.isVisible()) {
            ...
            const executeButton = page.locator(
                '[data-testid="execute-task"], button:has-text("실행")'
            );
            ...
            if (await executeButton.isVisible()) {
                ...
                const executionStatus = page.locator(
                    '[data-testid="execution-status"], [class*="execution-status"]'
                );
    ...
    test('should show execution progress', async ({ page }) => {
        const progressIndicator = page.locator(
            '[data-testid="execution-progress"], [class*="progress"]'
        );
    ...
    test('should show approve/reject buttons after execution', async ({ page }) => {
        const approveButton = page.locator(
            '[data-testid="approve-result"], button:has-text("승인"), button:has-text("Approve")'
        );
        const rejectButton = page.locator(
            '[data-testid="reject-result"], button:has-text("거부"), button:has-text("Reject")'
        );
    ...
    test('should move task to done on approval', async ({ page }) => {
        const approveButton = page.locator(
            '[data-testid="approve-result"], button:has-text("승인")'
        );
...
test.describe('Kanban Board Interactions', () => {
    ...
    test('should filter tasks by search query', async ({ page }) => {
        ...
        if (await searchInput.isVisible()) {
            ...
            // Verify filtering
            const _visibleTasks = page.locator('[data-task-id]:visible');
    ...
    test('should open add task modal', async ({ page }) => {
        const addButton = page
            .locator('[data-testid="add-task"], button:has-text("추가"), button:has-text("Add")')
            .first();
...
test.describe('Real-time Collaboration', () => {
    ...
    test('should show connected users indicator', async ({ page }) => {
        ...
        const usersIndicator = page.locator(
            '[data-testid="online-users"], [class*="collaboration"]'
        );
