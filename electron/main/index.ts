...
async function registerIpcHandlers(): Promise<void> {
  ...
  ipcMain.handle('projects:list', async (_event, filters?: { status?: ProjectStatus; isArchived?: boolean }) => {
    try {
...
ipcMain.handle('projects:create', async (_event, data: { title: string; description?: string; ownerId: number; baseDevFolder?: string | null; projectGuidelines?: string | null }) => {
    try {
  ...
  ipcMain.handle('projects:update', async (_event, id: number, data: Partial<{ title: string; description: string; status: string; baseDevFolder?: string | null; projectGuidelines?: string | null }>) => {
    try {
  ...
  ipcMain.handle('tasks:list', async (_event, projectId: number, filters?: { status?: string; priority?: string; assigneeId?: number }) => {
    try {
  ...
  ipcMain.handle('tasks:create', async (_event, data: Partial<NewTask> & { projectId: number; title: string }) => {
    try {
