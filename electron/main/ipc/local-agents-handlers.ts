 ...
 * Results are cached per app session to avoid repeated exec calls
 */
async function checkCommandInstalled(command: string): Promise<AgentCheckResult> {
    // Check cache first
    if (installationCache.has(command)) {
        console.log(
            `[LocalAgents] Using cached result for ${command}:`,
            installationCache.get(command)
        );
        return installationCache.get(command)!;
    }
    ...
    if (!allowedCommands.includes(command)) {
        ...
        installationCache.set(command, result);
        return result;
    }
    ...
    // Cache the result
    installationCache.set(command, result);
    console.log(`[LocalAgents] Cached installation status for ${command}:`, result);
...
export function registerLocalAgentsHandlers(mainWindow?: BrowserWindow | null): void {
    ...
    ipcMain.handle(
        'localAgents:checkInstalled',
        async (_event, command: string): Promise<AgentCheckResult> => {
            try {
    ...
    ipcMain.handle(
        'localAgents:launchInTerminal',
        async (_event, command: string): Promise<void> => {
            try {
    ...
    // Get session info
    ipcMain.handle(
        'localAgents:getSession',
        async (_event, sessionId: string): Promise<SessionInfo | null> => {
