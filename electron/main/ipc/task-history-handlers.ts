...
function transformHistoryEntry(record: any): TaskHistoryEntry {
  return {
    ...
    eventData: record.eventData ? (typeof record.eventData === 'string' ? JSON.parse(record.eventData) : record.eventData) : null,
...
export function registerTaskHistoryHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, taskId: number, limit?: number) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, taskId: number, eventType: TaskHistoryEventType) => {
      try {
        const entries = await taskHistoryRepository.findByTaskIdAndEventType(taskId, eventType);
        return entries.map(transformHistoryEntry);
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const entry = await taskHistoryRepository.create(taskId, eventType, eventData, metadata);
