...
export class TaskHistoryRepository {
  ...
  async logExecutionCompleted(
    taskId: number,
    content: string,
    metadata: { provider?: string; model?: string; cost?: number; tokens?: number; duration?: number },
    aiResult?: TaskHistoryEventData['aiResult']
  ): Promise<TaskHistory> {
    return this.create(
      taskId,
      'execution_completed',
      { content, aiResult, ...metadata },
      metadata
    );
  }
  ...
  async logPromptRefined(
    ...
    return this.create(
      taskId,
      'prompt_refined',
      { previousPrompt, newPrompt, refinementReason }
    );
  }
