...
import { projects, projectMembers, type Project, type NewProject } from '../schema';
...
export class ProjectRepository {
  ...
  async findAll(filters?: { status?: ProjectStatus; isArchived?: boolean }): Promise<Project[]> {
    const conditions = [
      eq(projects.isArchived, filters?.isArchived ?? false),
    ];
    ...
    return await db
      ...
      .where(
        and(
          ...
          or(like(projects.title, searchPattern), like(projects.description, searchPattern))
        )
      )
      .orderBy(desc(projects.updatedAt));
  }
