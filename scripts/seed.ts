...
async function seed() {
  ...
  try {
    ...
    const [project] = await db
      ...
      .values({
        title: 'AI Workflow Manager Development',
    ...
    const mainTasks = [
      // ========== 기획 단계 (완료) ==========
      {
        ...
        description: 'AI 워크플로우 매니저의 핵심 가치 제안과 주요 기능을 정의합니다. 타겟 사용자, 핵심 유즈케이스, 차별화 포인트를 명확히 합니다.',
      ...
      },
      {
        ...
        description: 'Notion, Linear, Jira 등 기존 프로젝트 관리 도구 분석. AI 기반 도구들(Cursor, GitHub Copilot Workspace 등) 벤치마킹.',
        status: 'done' as const,
      ...
      },
      {
        ...
        description: '주요 타겟 사용자(개발자, PM, 스타트업 창업자)별 페르소나를 정의하고 핵심 사용 시나리오를 작성합니다.',
        status: 'done' as const,
        priority: 'high' as const,
        order: 3,
        tags: ['기획', 'UX', 'persona'],
      ...
      // ========== 디자인 단계 (진행중/검토중) ==========
      {
        ...
        description: '색상 팔레트, 타이포그래피, 간격 시스템, 아이콘 세트 등 디자인 시스템을 정의하고 재사용 가능한 컴포넌트 라이브러리를 구축합니다.',
        status: 'done' as const,
        priority: 'high' as const,
        order: 4,
      ...
      },
      {
        ...
        description: '드래그앤드롭 칸반 보드의 와이어프레임과 고해상도 목업을 제작합니다. 태스크 카드, 컬럼, 필터링 UI를 포함합니다.',
      ...
      },
      {
        ...
        description: '프로젝트 진행률, AI 사용량, 비용 분석 등을 시각화하는 대시보드 UI를 디자인합니다.',
      ...
      {
        title: '모바일 반응형 디자인',
        description: '태블릿 및 모바일 환경에서의 사용성을 고려한 반응형 디자인을 적용합니다.',
      ...
      // ========== 개발 단계 - 백엔드 (일부 완료) ==========
      {
        ...
        description: '로컬 우선 아키텍처를 위한 SQLite 데이터베이스와 Drizzle ORM을 설정합니다. 마이그레이션 시스템 구축.',
        status: 'done' as const,
        priority: 'urgent' as const,
        order: 8,
      ...
      },
      {
        ...
        description: '다중 AI 프로바이더 지원을 위한 추상화 레이어 구현. API 키 관리, 모델 선택, 비용 추적 기능 포함.',
        status: 'done' as const,
        priority: 'high' as const,
        order: 9,
      ...
      },
      {
        ...
        description: 'Anthropic의 MCP를 활용한 도구 실행 환경 구축. 파일 시스템, Git, 터미널 도구 통합.',
        status: 'in_progress' as const,
        priority: 'high' as const,
        order: 10,
      ...
      },
      {
        ...
        description: 'Renderer와 Main 프로세스 간 안전한 통신 채널 구현. 타입 안전성과 에러 핸들링 포함.',
        status: 'in_review' as const,
        priority: 'high' as const,
        order: 11,
...
contextBridge.exposeInMainWorld('api', {
  ...
  // ... more handlers
      ...
      // ========== 개발 단계 - 프론트엔드 (진행중) ==========
      {
        ...
        description: 'Vue 3 Composition API, TypeScript 엄격 모드, Pinia 스토어 아키텍처 설정.',
        status: 'done' as const,
        priority: 'high' as const,
        order: 12,
      ...
      },
      {
        ...
        description: '태스크 생성, 조회, 수정, 삭제 기능과 Pinia 스토어를 통한 상태 관리 구현.',
        status: 'done' as const,
        priority: 'high' as const,
        order: 13,
      ...
      },
      {
        ...
        description: 'AI 에이전트 실행 시 실시간 스트리밍 출력 표시, 진행 상황 표시, 중간 결과 프리뷰 기능.',
        status: 'in_progress' as const,
        priority: 'urgent' as const,
        order: 14,
      ...
      },
      {
        ...
        description: '태스크 간 의존 관계를 시각적으로 표시하고 편집할 수 있는 그래프 UI 구현.',
        status: 'todo' as const,
        priority: 'medium' as const,
        order: 15,
      ...
      },
      {
        ...
        description: 'API 키 설정, 기본 AI 모델 선택, 비용 한도 설정 등 사용자 설정 페이지 구현.',
        status: 'in_review' as const,
        priority: 'medium' as const,
        order: 16,
      ...
      // ========== 테스트 및 QA ==========
      {
        ...
        description: 'Playwright를 사용한 E2E 테스트 환경 설정. 주요 사용자 플로우에 대한 테스트 시나리오 작성.',
        status: 'todo' as const,
        priority: 'medium' as const,
        order: 17,
      ...
      // ========== 배포 및 운영 ==========
      {
        ...
        description: 'electron-builder를 사용한 macOS/Windows/Linux 빌드 설정. 코드 서명 및 공증 프로세스 구축.',
        status: 'todo' as const,
        priority: 'high' as const,
        order: 19,
      ...
      },
      {
        ...
        description: 'electron-updater를 사용한 자동 업데이트 기능 구현. 업데이트 알림 UI 및 롤백 기능.',
        status: 'todo' as const,
        priority: 'medium' as const,
        order: 20,
    ...
    if (taskIdMap[dependentTaskTitle] && taskIdMap[dependsOnTaskTitle]) {
      ...
      console.log(`✅ Set dependency: "${dependentTaskTitle}" depends on "${dependsOnTaskTitle}"`);
    }
